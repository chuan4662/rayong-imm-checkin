# 90_ROADMAP_v2_PLAN.md — แผนพัฒนาระยะ 2 (Roadmap + สเปกเทคนิค)

> **เอกสารนี้คืออะไร:** แผนพัฒนาต่อจาก v1 จัดทำ 6 ก.ค. 2569 โดย agent วางแผน (Cowork project "AI fable วางแผน ปรับระบบ เว็บเช็คอิน") ตามคำสั่งเจ้าของโปรเจกต์ (ชวนชัย) — เจ้าของยืนยันทิศทางผ่านการถาม-ตอบแล้ว 3 ข้อ: (1) เป้าหมาย = ใช้ภายในให้เป็นเลิศ **และ** เตรียมขยายให้ ตม.อื่น เท่ากัน (2) ระดับเอกสาร = roadmap + สเปกเทคนิคพร้อมลงมือ (3) ระบบคะแนน/รางวัล = ออกแบบ framework รอไว้เลย
>
> **สำหรับ agent ที่รับไปรัน:** อ่านคู่กับ `CLAUDE.md` (สถานะจริงปัจจุบัน) และ `00_SPEC_v1_ORIGINAL.md` (สเปกตั้งต้น) เสมอ — ถ้าเอกสารนี้ขัดกับสถานะจริงใน `CLAUDE.md` (เช่น มี migration ใหม่กว่าที่เอกสารนี้อ้าง) ให้ยึด `CLAUDE.md` แล้วปรับเลข migration ตามจริง เอกสารนี้กำหนด "จะทำอะไร ลำดับไหน ออกแบบอย่างไร" ไม่ใช่บันทึกสถานะ

> **⚠️ (10 ก.ค. 2569) อัปเดตความคืบหน้า — P0.2 (git+CI) และ P0.1 (เก็บตกเล็ก) เสร็จแล้วทั้งคู่:** รายละเอียดเต็มอยู่ใน `CLAUDE.md` ข้อ 23 (P0.2) และข้อ 25 (P0.1) ตามลำดับ นอกจากนี้ยังมีงานเก็บตกเล็กเพิ่มเติมนอกแผนเดิม (cron timeout fix ใช้เลข migration 24 ไปแล้ว, GPS timeout quick win, UX quick win รอบ 2 — ดู `CLAUDE.md` ข้อ 24/26) ทำให้เอกสารนี้ต้องปรับ 2 จุด: **(ก)** เลขนัมเบอร์ migration ในข้อ 8 เลื่อนทั้งหมด (24→25, 25→26, 26→27, 27→28, 28→29) เพราะเลข 24 ถูกใช้ไปกับ cron timeout fix ที่ไม่ได้อยู่ในแผนเดิม **(ข)** เพิ่มหัวข้อ P0.5 (งาน UX เก็บตกจาก feedback เจ้าของรอบ 10 ก.ค.) เข้าไปในเฟส P0 — ดูรายละเอียดในข้อ 3 ด้านล่าง

---

## 0. หลักการคงที่ (Invariants) — สิ่งที่แผนนี้ไม่แตะ

เจ้าของให้อิสระเรื่องขอบเขตฟีเจอร์ (ยกเลิกการ freeze scope v1) **แต่กติกาความถูกต้องยังบังคับใช้ทุกข้อ:**

1. เวลาตัดสินสถานะ = server time (`now()`) เท่านั้น
2. คำนวณสถานะต้องแปลง Asia/Bangkok ก่อนเทียบเกณฑ์
3. บังคับกล้องสด (`getUserMedia`) ห้าม `<input type=file>`
4. GPS บันทึกอย่างเดียว ไม่บล็อกการเช็กอิน
5. 1 คน = 1 เช็กอิน/วัน บังคับที่ระดับ DB (unique constraint)
6. ~~ห้าม scope creep เกิน v1~~ → **ปรับเป็น:** scope ใหม่ = ตาม roadmap นี้ งานนอก roadmap ต้องถามเจ้าของก่อนเสมอ

**กติกา workflow ของโปรเจกต์ (CLAUDE.md ข้อ 2) ยังใช้ครบทุกข้อ:** ห้าม deploy ก่อนถาม, deploy ครบ 3 ไฟล์, ส่งไฟล์ให้ตรวจก่อน, verify จริงก่อนสรุปเสร็จ, เช็กไฟล์ตรงกับเว็บจริงก่อนแก้, ห้ามสร้างบัญชี Auth แทนเจ้าของ, อัปเดต `CLAUDE.md` ทุกครั้งที่งานเสร็จ

**เส้นแดงเดิมที่ยังอยู่:** ทุกฟีเจอร์ที่แตะประชาชน (ฟีดแบ็ก + QR เคาน์เตอร์) ต้องได้อนุมัติจากผู้บังคับบัญชาเป็นลายลักษณ์อักษรก่อนเริ่มเขียนโค้ด — ไม่มีข้อยกเว้น

---

## 1. วิเคราะห์สถาปัตยกรรมปัจจุบัน — จุดแข็งและความเสี่ยงเชิงโครงสร้าง

### จุดแข็ง (อย่าทำลาย)
- Vanilla HTML/JS ไม่มี build step → deploy ง่าย พังยาก agent รุ่นไหนมาแก้ต่อก็ได้
- ทุกการเขียน DB ผ่าน RPC SECURITY DEFINER เท่านั้น (ไม่มี direct insert/update จาก client) → ตรรกะรวมศูนย์ ตรวจสอบได้
- วินัย verify ก่อนสรุปเสร็จ + บันทึกบั๊กกันเจอซ้ำใน CLAUDE.md → คุณภาพส่งมอบสูงจริง

### ความเสี่ยงเชิงโครงสร้าง (เรียงตามความรุนแรง × โอกาสเกิด)

| # | ความเสี่ยง | หลักฐาน/ผลกระทบ |
|---|---|---|
| R1 | **ไม่มี version control** — Netlify Drop + ไฟล์ใน OneDrive คือ source of truth คู่ขนานที่เคย drift มาแล้ว | บั๊ก CLAUDE.md ข้อ 6.5 เกิดซ้ำหลายรอบ ถ้า deploy ทับด้วยไฟล์เก่า = ลบฟีเจอร์ production โดยไม่รู้ตัว |
| R2 | **โค้ดซ้ำ dashboard.html / report.html** — ตรรกะเดียวกันเขียน 2 ชุด (auth vs PIN) | ทุกฟีเจอร์จ่ายต้นทุน ×2 และเสี่ยง 2 ไฟล์ behave ไม่ตรงกัน (เคยเกิด: คอลัมน์สิทธิ์หัวหน้ามีใน dashboard แต่ไม่มีใน report) |
| R3 | **PIN ผ่าน anon RPC ไม่มี rate limit** — ยิง `do_check_in`/`do_get_my_month_stats` เดา PIN ได้ไม่จำกัด | PIN 4-6 หลัก brute force ได้ในหลักชั่วโมง เสี่ยงทั้งเช็กอินแทนกันและอ่านสถิติคนอื่น |
| R4 | **ไม่มี backup ประจำ** — Supabase free tier ไม่มี point-in-time recovery | ถ้า migration พลาด/ลบข้อมูลผิด = ข้อมูลเช็กอินจริงหายถาวร |
| R5 | **ค่าเฉพาะ ตม.ระยอง hardcode กระจายอยู่ในโค้ด** — ชื่อหน่วยใน UI, รัศมี 50 ม. ในตัว RPC, รายชื่อ/กลุ่มงานใน SQL | ขวางทาง Track B โดยตรง ทุกไฟล์ที่ clone ไปหน่วยอื่นต้องไล่แก้มือ |

แผนด้านล่างออกแบบให้งาน Track B (เตรียมขยาย) ส่วนใหญ่ = การแก้ R1–R5 ซึ่ง**ได้ประโยชน์กับ ตม.ระยองเองทันทีด้วย** ไม่ใช่งานลงทุนเปล่ารออนาคต

---

## 2. โครงแผน — 2 Track / 4 เฟส

```
Track A (ใช้ภายในให้เป็นเลิศ)        Track B (โครงสร้างเพื่อขยาย)
──────────────────────────────      ──────────────────────────────
P0  เก็บตก + อุดความเสี่ยงเร่งด่วน   ← ทำร่วมกัน (ก.ค. สัปดาห์ 2-3)
P1  Badge ทีม (เริ่ม ~20 ก.ค.)       P1' วางราก work_group (ทำพร้อม P1)
P2  คะแนน/รางวัล + แผนก + สิทธิ์     (ส.ค. — หลังข้อมูลครบเดือน 3 ส.ค.)
P3  ─                               P3 Multi-office packaging (ก.ย.+)
```

**หลักการจัดลำดับ:** งานที่ปลดล็อกงานอื่น (foundation) มาก่อน, งานที่มีกำหนดจากเจ้าของ (badge ทีม ~20 ก.ค., คะแนน ~3 ส.ค.) ยึดตามกำหนด, งานเสี่ยงสูงทำตอนระบบมี git + backup แล้ว

---

## 3. เฟส P0 — เก็บตก + อุดความเสี่ยง (ก.ค. สัปดาห์ 2-3, ก่อน 20 ก.ค.)

### P0.1 งานเก็บตกเล็ก (ครึ่งวัน — quick win) — ✅ เสร็จแล้ว 10 ก.ค. 2569
- `report.html`: เพิ่มคอลัมน์ "สิทธิ์หัวหน้า" ในตารางรายชื่อ ให้เท่ากับ `dashboard.html` (ข้อมูลมีอยู่แล้วใน RPC ที่เรียกอยู่ แค่ render เพิ่ม)
- error-message mapping ของปุ่มกันลบรูป (`do_set_retention_hold`/`do_supervisor_set_retention_hold`): map ทุก error code ที่ RPC คืนได้ → ข้อความไทย ไม่โชว์โค้ดดิบ
- ไม่มี migration ใหม่ ทั้งคู่เป็น frontend ล้วน deploy รวมกับ P0 อื่นได้
- **สถานะ: push แล้ว (commit `37827bc`) verify บนเว็บจริงผ่านหมด — รายละเอียดเต็ม `CLAUDE.md` ข้อ 25**

### P0.2 Git + Netlify CI — แก้ R1 (สำคัญที่สุดของ P0) — ✅ เสร็จแล้ว 9 ก.ค. 2569
**สิ่งที่ทำ:**
1. เจ้าของสร้าง GitHub repo แบบ **private** (agent ห้ามสร้างบัญชี/login แทน — เหมือนกติกา Supabase Auth) แนะนำชื่อ `rayong-imm-checkin`
2. โครง repo: `index.html`, `dashboard.html`, `report.html`, โฟลเดอร์ `migrations/` (ย้ายไฟล์ .sql 01–23 เข้าไป), `edge-functions/`, `docs/` (CLAUDE.md, 00_SPEC, เอกสารนี้)
3. ผูก Netlify site `comfy-gaufre-b6b83e` กับ repo (Site settings → Build & deploy → Link repository, ไม่มี build command, publish directory = root)
4. **กติกาใหม่หลังผูก git:** `git push` = deploy อัตโนมัติ ดังนั้นกติกา "ห้าม deploy ก่อนถาม" แปลงร่างเป็น **"ห้าม push ก่อนถาม"** — commit ในเครื่องได้ตลอด, push ต้องรอ confirm และได้ **Deploy Preview URL ฟรี** จาก branch/PR ซึ่งแก้ข้อจำกัดเดิมที่ไม่มี staging ให้เจ้าของตรวจก่อนขึ้นจริง
5. อัปเดต CLAUDE.md ข้อ 2.3 (ไม่ต้องอัปโหลด 3 ไฟล์มือแล้ว) และข้อ 2.6 (source of truth = git ไม่ใช่การ fetch เทียบเว็บ)

**เกณฑ์ผ่าน:** push commit ทดสอบ (แก้ comment 1 บรรทัด) → Netlify auto-deploy สำเร็จ → เว็บจริงไม่มี regression → rollback ผ่าน Netlify UI ได้

**ผลจริง vs แผน (เบี่ยงเบน 2 จุด, บันทึกไว้กันสับสน):** (1) repo สร้างเป็น **public** ไม่ใช่ private ตามแผนเดิม — เจอบั๊ก "Unrecognized Git contributor" เพราะแพลน Netlify Personal ไม่รองรับ private repo ที่ push ผ่าน PAT ทางแก้คือเปลี่ยน repo เป็น public แทนการอัปเกรดแพลน (รายละเอียด `CLAUDE.md` ข้อ 23.3) (2) **ไม่ได้ใช้ branch/PR + Deploy Preview URL** ตามที่แผนนี้เสนอไว้ในข้อ 4 ด้านบน — โปรเจกต์ตัดสินใจ push ตรงไป `main` เสมอเหมือนเดิม (เหตุผล: workflow เดิมของเจ้าของคือแก้เสร็จ→ส่งไฟล์จากเครื่องให้ตรวจ→confirm→push อยู่แล้ว ไม่ต้องพึ่ง preview URL เพิ่ม) ยังไม่ปิดโอกาสเปลี่ยนมาใช้ PR flow ในอนาคตถ้าจำเป็น — รายละเอียดเต็ม `CLAUDE.md` ข้อ 23 ทั้งหมด (23.1–23.7)

### P0.3 PIN rate limiting — แก้ R3 (migration 25) — ✅ เสร็จสมบูรณ์ 10 ก.ค. 2569 (SQL รันจริง + ทดสอบ E2E ผ่านหมด + แก้ frontend เสร็จ รอ push — รายละเอียดเต็ม `CLAUDE.md` ข้อ 27)
- ✅ เพิ่มคอลัมน์ `officer.pin_fail_count int default 0`, `officer.pin_locked_until timestamptz`
- ✅ สร้างฟังก์ชันกลาง `check_and_count_pin(p_officer_id, p_pin)` — ผิด → count+1, ครบ 5 ครั้ง → ล็อก 15 นาที, ถูก → reset count เป็น 0, ระหว่างล็อก → คืน error `pin_locked` พร้อมเวลาปลด (แม้ PIN ถูกก็ยังถูกบล็อกจนพ้นเวลา — ทดสอบยืนยันแล้ว)
- ✅ ทั้ง 21 ฟังก์ชันที่เช็ก PIN จริง ครอบด้วย thin wrapper แทนแก้ business logic ตรงๆ (RENAME เดิม → `_impl`, REVOKE จาก anon/authenticated, สร้าง wrapper ใหม่ชื่อเดิมเรียก `check_and_count_pin` ก่อนเสมอ) — ดูรายชื่อเต็ม + เหตุผลใน `CLAUDE.md` ข้อ 27.1 และไฟล์ `25_pin_rate_limiting.sql`
- ✅ frontend `index.html`/`report.html` ทุกจุดที่กรอก PIN: map error `pin_locked` → ข้อความไทยบอกเวลาที่เหลือ (`dashboard.html` ไม่ต้องแก้ เพราะ auth-based ล้วน ไม่มี RPC รับ `p_pin`)
- ✅ แก้ช่องโหว่เพิ่มเติมที่พบระหว่างทำ: `do_reset_pin`/`do_supervisor_reset_pin_impl` เดิมไม่ clear `pin_fail_count`/`pin_locked_until` ตอนรีเซ็ต PIN ทำให้ปลดล็อกไม่ได้ผลจริง — แก้แล้ว
- **บทเรียนสำคัญที่ค้นพบระหว่างทำ (กระทบ P0.4):** bash/curl ของ agent sandbox เข้าถึง Supabase ไม่ได้เลย (ติด allowlist proxy) ต้องใช้ Chrome `fetch()` แทนเสมอสำหรับงานที่ต้องเรียก Supabase REST จากภายนอก SQL Editor — ดู `CLAUDE.md` ข้อ 27.3/27.6 ก่อนเริ่ม P0.4

**เกณฑ์ผ่าน:** ✅ ใส่ PIN ผิด 5 ครั้ง → ล็อก, ครั้งที่ 6 แม้ถูกก็ถูกปฏิเสธจนพ้นเวลา, PIN ถูกก่อนครบ 5 → count reset, รีเซ็ต PIN → ปลดล็อก — ทดสอบด้วยแถวทดสอบชั่วคราวแล้วลบทิ้งตามวินัยเดิม ผ่านทุกเกณฑ์จริง

### P0.4 Backup ประจำ — แก้ R4 (**⏸️ เจ้าของสั่งพักไว้ก่อน 11 ก.ค. 2569 — ห้ามเริ่มทำเองก่อนถึงเงื่อนไขปลดล็อก**)

**✅ (11 ก.ค. 2569) ตรวจสอบสดจากหน้า Supabase Dashboard ยืนยันแล้ว:** องค์กร "Chuan" อยู่บนแพลน **Free** และหน้า Database Backups ระบุตรงๆ ว่า **"Free Plan does not include project backups"** — ตอนนี้ระบบไม่มีการสำรองข้อมูลอัตโนมัติใดๆ เลยแม้แต่นิดเดียว (ไม่มี scheduled backup, ไม่มี point-in-time recovery) เป็นความเสี่ยงจริงที่ยังเปิดอยู่ ไม่ใช่แค่สมมติฐาน — ยืนยันราคา Pro plan ปัจจุบัน = **$25/เดือน** (ได้ daily backup อัตโนมัติ เก็บย้อนหลัง 7 วัน กู้คืนเองได้จากหน้าเว็บ ไม่ต้องเขียนโค้ดเพิ่ม)

**การตัดสินใจของเจ้าของ (11 ก.ค. 2569, ถามตรงๆ ผ่าน AskUserQuestion แล้วเลือกเอง):** "ยังไม่ตัดสินใจตอนนี้ ควรมีระบบ backup เมื่อมีการขยายการใช้งานแผนอื่นๆเพิ่มเติม หรือให้ตม.จังหวัดอื่นๆใช้งาน" — สรุปคือ **ผูก P0.4 เข้ากับเงื่อนไข P3 (multi-office) แทนที่จะทำตามกำหนดเดิมใน P0** เจ้าของรับความเสี่ยง "ไม่มี backup" ระหว่างนี้โดยตั้งใจ (ข้อมูลสเกลเล็ก 19 คน หน่วยเดียว) **agent ตัวถัดไปห้ามเริ่มงาน P0.4 เองก่อนที่เจ้าของจะหยิบยกเรื่องขยายไปหน่วยอื่น/แผนอื่นขึ้นมาก่อน** เหมือนกติกาเดิมที่ใช้กับ P1 badge (ห้ามเริ่มก่อนกำหนดเว้นแต่เจ้าของหยิบยกเอง)

**เมื่อถึงเวลาต้องทำจริง (อ้างอิงสำหรับ agent ตัวถัดไป) — ทางเลือก 2 แบบที่เคยประเมินไว้:**
1. **อัปเกรด Supabase เป็น Pro plan ($25/เดือน)** — แนะนำเป็นค่าเริ่มต้นเมื่อถึงเงื่อนไข เพราะเชื่อถือได้กว่า ไม่ต้องดูแลโค้ดเอง Supabase จัดการให้ทั้งหมด เหมาะกับตอนขยายสเกล/ข้อมูลสำคัญขึ้น
2. **DIY scheduled task export CSV** — ยังมีข้อจำกัดทางเทคนิคที่ยังไม่ได้แก้: bash sandbox ของ agent เข้าถึง Supabase ไม่ได้ตรงๆ (ติด allowlist, ดู `CLAUDE.md` ข้อ 27.3/27.6) ต้องอ้อมผ่านเบราว์เซอร์ ยังไม่เคยทดสอบว่ารันแบบอัตโนมัติไม่มีคนคอยได้จริงหรือไม่ — ถ้าจะเลือกทางนี้ ต้องทดสอบความเป็นไปได้ก่อนเริ่มออกแบบจริง
3. **⚠️ ข้อควรระวังเพิ่มเติมถ้าเลือกทาง DIY:** repo เป็น **public** แล้ว (ตั้งแต่ P0.2) ห้าม backup ข้อมูลเช็กอินจริงลง repo เดิมเด็ดขาด เพราะจะเปิดเผยข้อมูลเจ้าหน้าที่ต่อสาธารณะ ต้องเก็บ backup ไว้ที่อื่น (โฟลเดอร์ Cowork outputs/OneDrive หรือ private repo แยกต่างหาก)

### P0.5 UX quick win เพิ่มเติมจาก feedback เจ้าของ (บันทึกเพิ่ม 10 ก.ค. 2569 — นอกแผนเดิม แต่เข้าเกณฑ์ P0 เพราะต้นทุนต่ำ ไม่แตะ RPC/schema)

เจ้าของรีวิวการใช้งานจริงแล้วส่ง feedback รายหน้า แบ่งเป็นทำแล้ว/ยังไม่ทำ:

**✅ ทำแล้ว — `index.html` (push แล้ว, รายละเอียดเต็ม `CLAUDE.md` ข้อ 26):**
- ขยาย GPS geolocation timeout 8 วิ → 20 วิ (commit `f58cd48`) — ลดม่วง "เช็กอินไม่สมบูรณ์" ปลอมจาก GPS ตอบช้า
- จำชื่อล่าสุดที่เลือกใน `localStorage` เป็นค่า default ครั้งถัดไป (commit `5f7cac2`)
- ปุ่มลัด "⚡ ปฏิบัติงานปกติ" กรอกหมายเหตุอัตโนมัติ (commit `5f7cac2`)
- แสดงสถานะ GPS (🟡/🟢/🔴/⚪) ในหน้า "ตรวจสอบก่อนยืนยัน" ไม่ใช่แค่ตอน modal ม่วงเด้ง (commit `5f7cac2`)

**📋 ยังไม่ทำ — รอคิว:**
- **Dashboard:** เมนูลอย/จัดกลุ่มการ์ดเป็น "ประจำวัน / รายงาน / ตั้งค่า" (ตอนนี้มี 11 การ์ดหน้าเดียว scroll ยาว) — ต้นทุนต่ำมาก (CSS/anchor link ล้วน ไม่แตะ RPC) แนะนำทำก่อนเริ่ม P1 เพราะการ์ดใหม่ (team coverage) จะเพิ่มเป็นการ์ดที่ 12 ยิ่งรกกว่าเดิมถ้าไม่จัดกลุ่มก่อน
- **Report.html:** ตาราง 9-10 คอลัมน์อ่านยากบนมือถือแนวตั้ง — ระยะสั้นแนะนำหัวหน้าใช้แนวนอน/คอมก่อน ระยะยาวค่อยทำ card layout (งานใหญ่กว่า ต้อง redesign ทั้งหน้า ไม่ใช่ quick win แล้ว — ควรทำแยกเป็นงานเดี่ยวไม่พ่วง P0)

---

## 4. เฟส P1 — Badge ระดับทีม + วางราก work_group (เริ่ม ~20 ก.ค. ตามกำหนดที่เจ้าของขอ)

### ⚠️ การปรับดีไซน์จากที่ยืนยันไว้ (ต้องแจ้งเจ้าของก่อนเริ่ม)
ดีไซน์เดิม (CLAUDE.md ข้อ 22) ระบุ "เพิ่มคอลัมน์ `officer.team_name` text พอ" — **แผนนี้เสนอเปลี่ยนเป็นตาราง `work_group` ตั้งแต่แรก** เพราะฟีเจอร์เฟส P2 อีก 2 ตัว (ระบบแผนก + สิทธิ์หัวหน้าตามแผนก) ต้องการโครงสร้าง "กลุ่มงาน" แบบเดียวกันเป๊ะ ถ้าใช้ text ตอนนี้จะต้อง migrate ซ้ำใน 1 เดือน ต้นทุนเพิ่มขึ้นเปล่าๆ ส่วนตรรกะ badge/เกณฑ์เวลา/UI คงตามดีไซน์ที่ยืนยันแล้วทุกอย่าง — ถ้าเจ้าของไม่เห็นด้วย ให้ถอยกลับไปใช้ `team_name` text ตามดีไซน์เดิมได้โดย badge ทีมไม่กระทบ

### P1.1 Backend (migration 27 — เดิมเขียนว่า 25 แล้วเลื่อนเป็น 26 เพราะ 24 ถูกใช้ไปกับ cron timeout fix เลื่อนอีกรอบเป็น 27 เพราะ 26 ถูกใช้ไปกับ hotfix แก้บั๊ก PIN rate limiting บล็อกหัวหน้า PIN แล้ว ดูข้อ 8)
```sql
create table public.work_group (
  id          uuid primary key default gen_random_uuid(),
  name        text not null unique,          -- 'กลุ่มงานธุรกิจ', 'กลุ่มงานครอบครัว', ...
  is_ot_team  boolean not null default false, -- true = เข้าเกณฑ์ badge ทีม (คู่ OT)
  created_at  timestamptz not null default now()
);
alter table public.officer add column work_group_id uuid references public.work_group(id);
-- seed: insert กลุ่มงานธุรกิจ + ครอบครัว (is_ot_team=true) แล้ว update officer 4 คนที่เกี่ยวข้อง
--       (รายชื่อจริงให้ถามเจ้าของตอนรัน — อย่าเดา)
alter table public.settings add column team_ok_before   time not null default '08:20';
alter table public.settings add column team_warn_before time not null default '09:00';
```
- RPC `do_get_team_coverage()` (auth) / `do_supervisor_get_team_coverage(p_officer_id, p_pin)` (PIN): ต่อ work_group ที่ `is_ot_team=true` คืน `{group_name, first_checkin_time, member_names[], badge}` โดย badge = `ok` (มีคนเช็กอินก่อน `team_ok_before`) / `warn` (มีคนก่อน `team_warn_before`) / `none` (เลยแล้วยังไม่มีใคร — แสดงเฉพาะเมื่อเวลาปัจจุบัน Bangkok > `team_warn_before`)
- คำนวณจากเวลาเช็กอิน**จริง** (`checked_in_at`) ไม่ใช่ effective status — override สถานะสีไม่ควรเปลี่ยนข้อเท็จจริงว่าจุดบริการมีคนตอนกี่โมง
- แก้ `do_list_officers_admin`/`do_supervisor_list_officers` คืน `work_group_id`+ชื่อกลุ่มเพิ่ม (DROP ก่อน — เปลี่ยน return columns ตามบั๊กคลาสสิก 6.1)
- แก้ `do_get_settings`/`do_set_settings` (migration 22) รองรับ 2 ค่าเวลาใหม่

### P1.2 Frontend
- `dashboard.html`/`report.html`: การ์ด "🤝 จุดบริการกลุ่มงาน OT" วางเหนือการ์ด "ขาดเช็กอิน" แสดง badge ต่อกลุ่ม + เวลาคนแรกเช็กอิน — **ไม่แตะสีรายคนใดๆ ทั้งสิ้น** (ตามดีไซน์ยืนยัน)
- `dashboard.html` (เฉพาะชวนชัย): จัดการกลุ่มงานในการ์ดตั้งค่าระบบ — เพิ่ม/แก้ชื่อกลุ่ม, ติ๊ก `is_ot_team`, มอบสมาชิก (dropdown ต่อ officer ในตารางรายชื่อ)
- `index.html`: ไม่แตะ

**เกณฑ์ผ่าน:** จำลอง 3 เคส (คนแรกก่อน 08:20 / ระหว่าง 08:20–09:00 / ไม่มีใครถึง 09:01) ด้วยแถวทดสอบชั่วคราว badge ถูกทั้ง 3, สีรายคนของสมาชิกทีมไม่เปลี่ยนจากเดิมแม้แต่เคสเดียว, ลบข้อมูลทดสอบครบ

---

### P1.3 ระบบบัตรอนุญาตมาสาย (Grace Card) — ดีไซน์ยืนยันกับเจ้าของแล้ว 11 ก.ค. 2569

**ที่มา:** เจ้าของเสนอไอเดียเอง (ไม่ใช่ agent คิด) ระหว่างปรึกษาแผน P1 — ต้องการระบบ "บัตรรางวัล" ให้เจ้าหน้าที่ใช้เลื่อนเวลาเช็กอินสายได้แบบไม่ถูกนับว่าสาย เป็นแรงจูงใจเชิงบวกคู่กับ badge ทีม OT ที่วางแผนไว้เดิม ออกแบบผ่านการถาม-ตอบหลายรอบกับเจ้าของจนได้ข้อสรุปที่นิ่งแล้วทุกจุด **ยังไม่เขียนโค้ด รอถึงกำหนด P1 (~20 ก.ค.) เหมือนกับ badge ทีม**

**กติกาที่ยืนยันแล้ว (ห้าม agent ตีความเองถ้าจุดไหนไม่ชัด ให้ถามเจ้าของก่อนเขียนโค้ดจริง):**

1. **ทุกคน (19 คน) ต้องมีกลุ่ม ไม่ใช่แค่ 2 ทีม OT เดิม** — ขยาย `work_group`/`officer.work_group_id` จาก P1.1 ให้ครอบคลุมทุกคน ไม่จำกัดแค่ `is_ot_team=true` เจ้าของมีการแบ่งกลุ่มในใจอยู่แล้ว **แต่ต้องการให้ UI จัดการสมาชิกกลุ่มได้เอง (เพิ่ม/ย้ายคนภายหลังได้)** ไม่ hardcode — ใช้การ์ดจัดการกลุ่มงานที่มีอยู่แล้วใน P1.2 (ตอนรันจริงค่อยถามเจ้าของว่าจะแบ่ง 19 คนเป็นกี่กลุ่ม กลุ่มละใคร อย่าเดา)
2. **สิทธิ์การ์ดคือของ "กลุ่ม" ไม่ใช่ของแต่ละคน** — แต่ละกลุ่มใช้การ์ดได้สูงสุด **1 สิทธิ์/วัน** ไม่ว่ากลุ่มจะมีกี่คน บังคับด้วย unique index ระดับ DB บน (`work_group_id`, `target_date`) กันชนกันแม้ 2 คนกดพร้อมกันจริง (แพทเทิร์นเดียวกับที่บังคับ "1 คน/1 เช็กอิน/วัน" อยู่แล้ว)
3. **ยอดบัตรเป็นของรายคน มาจาก 2 แหล่ง:** (ก) แจกอัตโนมัติทุกเดือน (จำนวน/เดือนเท่าไหร่ให้เจ้าของกำหนดตอนรันจริง 1-2 ใบ) (ข) หัวหน้ากดมอบเพิ่มเป็นรางวัลได้ทุกเมื่อ (บันทึก `granted_by` + เวลา + หมายเหตุ เพื่อความโปร่งใส) **ไม่สะสมข้ามเดือน หมดอายุทุกสิ้นเดือน**
4. **ครอบคลุมเฉพาะ "มาสายแต่ยังเช็กอิน" เท่านั้น ไม่ครอบคลุม "ขาดเช็กอินเต็มวัน"** — ยังต้องเช็กอินจริงเสมอ เกณฑ์เวลา "สายไม่เกินกี่โมงถึงใช้บัตรได้" ตั้งเป็นค่าปรับได้ใน settings (`grace_card_late_cutoff`, default `09:30`) ไม่ hardcode
5. **ใช้สิทธิ์ได้ 2 ทาง:**
   - **จองล่วงหน้า** ไม่เกิน 3 วันถัดไป (T+1 ถึง T+3) — **หักยอดบัตรทันทีตอนกดจอง** (ไม่ใช่ตอนเช็กอินจริง) เหตุผลที่เจ้าของยืนยัน: ทำให้การจองมีน้ำหนักจริง วางแผนได้ว่าวันนั้นตื่นสายได้ คนในกลุ่มเห็นแล้วตกลงกันเองได้ หัวหน้าขยับคนไปคุมงานแทนได้เหมาะสม
   - **วอล์กอิน** ตอนเช็กอินสายจริงในวันนั้น ถ้ากลุ่มยังไม่มีใครจอง/ใช้สิทธิ์ของวันนั้นไปก่อน
6. **ยกเลิกการจองได้ แต่ล็อกก่อนถึงเวลา** — ยกเลิกไม่ได้ถ้าเหลือเวลาน้อยกว่า 12 ชั่วโมงก่อน **08:00 ของวันที่จองไว้** (= ล็อกตั้งแต่ 20:00 ของคืนก่อนหน้าเป็นต้นไป) ใช้ 08:00 เป็นเวลาอ้างอิงคงที่ (ตั้งเป็นค่าปรับได้ใน settings เช่นกัน `grace_card_lock_reference_time`) เพราะเวลาเช็กอินจริงยังไม่เกิดขึ้นล่วงหน้า ยกเลิกสำเร็จ = คืนยอดบัตร + ปลดสิทธิ์ของวันนั้นให้กลุ่มจองใหม่ได้
   - **⚠️ ต้องแสดงเวลา deadline ยกเลิกให้เห็นชัดเจนในหน้าจอเสมอ** (จุดที่เจ้าของเน้นย้ำ 11 ก.ค. 2569) — ทุกที่ที่แสดงการจอง (ทั้งฝั่งเจ้าของการจองเองและฝั่งเพื่อนร่วมกลุ่มที่เห็นการจอง) ต้องมีข้อความ "ยกเลิกได้ถึง [วันที่+เวลา 20:00]" กำกับไว้ด้วย ไม่ใช่ให้กดยกเลิกแล้วค่อยเจอ error เฉยๆ
7. **คนในกลุ่มต้องเห็นกันว่าใครจองวันไหนแล้ว** — ต้องมี UI ใน `index.html` (ฝั่งเจ้าหน้าที่เอง ไม่ใช่แค่ dashboard/report ฝั่งหัวหน้า) แสดงรายการจองของกลุ่มตัวเอง เพื่อให้ตกลง/สลับกันใช้สิทธิ์เองได้โดยไม่ต้องผ่านหัวหน้า
8. **แสดงผลเป็น badge พิเศษคู่กับสีสถานะจริง** — ห้ามเขียนสถานะทับเป็นเขียวเงียบๆ จนแยกไม่ออกว่ามาตรงเวลาจริงหรือใช้สิทธิ์ ต้องมี badge "🎫 ใช้บัตร" ให้เห็นชัดคู่กับสีเดิมเสมอ (สำคัญมากถ้าจะทำ P2 scoring ต่อ เพราะต้องแยกสถิติ "เขียวจริง" กับ "ใช้บัตร" ออกจากกันได้)

**สถาปัตยกรรมที่แนะนำ (ลดความเสี่ยง ไม่สร้างระบบใหม่ทั้งหมด):** ใช้กลไก `override_status`/`override_reason`/`override_by` ที่มีอยู่แล้ว (migration 17) เป็นฐาน — ตอนระบบ apply บัตรให้อัตโนมัติ (ไม่ใช่หัวหน้ากดเอง) ให้ตั้ง `override_status` เป็นสถานะที่ไม่นับว่าสาย + `override_reason = "ใช้บัตรอนุญาตมาสาย"` + เพิ่มคอลัมน์ใหม่ `check_in.grace_card_used boolean` เพื่อให้ฝั่ง UI แยก render เป็น badge 🎫 แทนข้อความ override ทั่วไปได้ทันที (โค้ด `renderCheckinTable`/`effective_status` ที่มีอยู่แล้วรองรับ `override_status` อยู่แล้ว แทบไม่ต้องแก้ query เดิม) — เท่ากับเปิดช่องทาง "self-override" ใหม่ (เดิม override ทำได้แค่หัวหน้า) ต้องออกแบบ RPC แยกเฉพาะสำหรับ flow นี้ ไม่เปิดสิทธิ์ override ทั่วไปให้เจ้าหน้าที่

**ตารางที่ต้องเพิ่ม (ร่าง ไม่ใช่ final SQL — รายละเอียดจริงเขียนตอนเริ่ม P1):**
```sql
-- ยอดบัตรที่ได้รับ (ต่อคน ต่อเดือน ต่อครั้งที่ได้รับ)
create table public.grace_card_ledger (
  id            uuid primary key default gen_random_uuid(),
  officer_id    uuid not null references public.officer(id),
  period_month  date not null,               -- วันที่ 1 ของเดือนนั้น
  source        text not null check (source in ('auto','manual')),
  granted_by    uuid references public.officer(id),   -- null ถ้า auto
  granted_at    timestamptz not null default now(),
  note          text
);

-- การจอง/ใช้สิทธิ์ของกลุ่ม ต่อวัน
create table public.grace_card_usage (
  id              uuid primary key default gen_random_uuid(),
  work_group_id   uuid not null references public.work_group(id),
  target_date     date not null,
  officer_id      uuid not null references public.officer(id),
  status          text not null default 'reserved' check (status in ('reserved','applied','cancelled')),
  reserved_at     timestamptz not null default now(),
  cancelled_at    timestamptz,
  check_in_id     uuid references public.check_in(id)  -- ผูกตอน apply จริงตอนเช็กอิน (เคสวอล์กอิน)
);
-- ⚠️ ห้ามใช้ unique constraint ธรรมดาบน (work_group_id, target_date) เพราะจะกันไม่ให้จองซ้ำวันเดิมได้แม้ยกเลิกไปแล้ว
-- ต้องใช้ partial unique index กรองเฉพาะที่ยังไม่ถูกยกเลิก:
create unique index grace_card_usage_active_per_group_day
  on public.grace_card_usage (work_group_id, target_date)
  where status <> 'cancelled';

alter table public.settings add column grace_card_late_cutoff time not null default '09:30';
alter table public.settings add column grace_card_lock_reference_time time not null default '08:00';
alter table public.check_in add column grace_card_used boolean not null default false;
```

**RPC ที่ต้องมี (ร่างชื่อ ยังไม่ใช่ signature สุดท้าย):**
- `do_reserve_grace_card(p_officer_id, p_pin, p_target_date)` — เช็กยอดคงเหลือ > 0, `target_date` อยู่ใน T+1..T+3, กลุ่มยังไม่มีสิทธิ์ถูกจอง/ใช้ของวันนั้น (ชน partial unique index ก็ error สวยๆ) → หักยอด (insert `grace_card_ledger` ไม่ต้อง insert แค่ insert `grace_card_usage` status='reserved' ก็พอ เพราะยอดคงเหลือคำนวณจาก ledger เดือนนั้น - usage ที่ status≠cancelled ของเดือนนั้น)
- `do_cancel_grace_card_reservation(p_officer_id, p_pin, p_usage_id)` — เช็ก `status='reserved'` และ `now() < (target_date + grace_card_lock_reference_time - interval '12 hours')` ก่อนอนุญาต → set `status='cancelled'`
- `do_get_my_grace_card_status(p_officer_id, p_pin)` — คืนยอดคงเหลือเดือนนี้ + รายการจองของกลุ่มตัวเอง (ทุกคนในกลุ่มเห็นเหมือนกัน) พร้อม deadline ยกเลิกของแต่ละรายการ (คำนวณจาก reference time ด้านบน) สำหรับ render ใน `index.html`
- แก้ `do_check_in`: ถ้าเวลาที่เช็กอินจริงอยู่ในช่วงสาย-ถึง-`grace_card_late_cutoff` → เช็กว่ากลุ่มมีสิทธิ์ที่ตัวเองจองไว้สำหรับวันนี้ไหม (auto-apply) หรือถ้ายังไม่มีใครจอง/ใช้เลย ให้ client ส่ง flag ว่าจะใช้แบบวอล์กอินมาด้วย → ถ้าผ่านเงื่อนไข insert/update `grace_card_usage` (status='applied', ผูก `check_in_id`) + ตั้ง `override_status`/`override_reason`/`grace_card_used=true` บนแถว check_in ใหม่
- หัวหน้ามอบบัตรเพิ่ม: ต่อยอด RPC ตระกูล `do_supervisor_*`/auth เดิม เพิ่ม `do_grant_grace_card(p_target_officer_id, p_count, p_note)` (เฉพาะ auth) / เวอร์ชัน PIN คู่กัน

**สิ่งที่ต้องรู้ล่วงหน้า (แจ้งเจ้าของไว้ ไม่ใช่ปัญหาต้องแก้ตอนนี้):** ถ้าเจ้าหน้าที่จองไว้แต่สุดท้ายมาตรงเวลา (ไม่ได้ใช้จริง) ยอดบัตรที่หักไปตอนจองจะไม่ได้คืนอัตโนมัติ (นอกจากกดยกเลิกทันก่อนล็อก) — เป็นผลตามดีไซน์ที่ยืนยันแล้ว (ข้อ 5 ด้านบน) ไม่ใช่บั๊ก

**หมายเหตุเลข migration:** ยังไม่ล็อกเลขตายตัวตอนนี้ (โปรเจกต์นี้เจอปัญหาเลขเลื่อนซ้ำแล้วหลายรอบ ดูข้อ 8) ให้เช็กเลขจริงจาก `CLAUDE.md` ตอนเริ่มเขียนโค้ด P1 จริงแทน

---

### P1.4 เช็คเอาท์ตอนเย็น (ไม่บังคับ, ให้รางวัลอย่างเดียว) — ดีไซน์ยืนยันกับเจ้าของแล้ว 11 ก.ค. 2569

**ที่มา:** เจ้าของอยากออกแบบคู่กับ badge ทีม OT (P1.1) และ Grace Card (P1.3) เพื่อเก็บข้อมูลชั่วโมงทำงานจริงตอนเย็น — สเปก v1 เดิมกันเรื่องเช็คเอาท์ไว้นอกขอบเขตโดยตั้งใจ (ห้าม scope creep) รอบนี้เป็นการรื้อฟื้นแบบมีขอบเขตชัดเจนตามที่เจ้าของยืนยัน **ออกแบบไว้ตอนนี้ ไม่ผูกกับกำหนด P1 (~20 ก.ค.) เจ้าของตั้งใจให้ทยอยขึ้นทีหลังก็ได้** ยังไม่เขียนโค้ด

**⚠️ ข้อสังเกตเชิงสถาปัตยกรรมสำคัญ:** ฟีเจอร์นี้แยกเป็น 2 ชั้นที่ไม่ต้องพึ่งกัน — (1) "บันทึกเช็คเอาท์ส่วนบุคคล" ไม่ต้องพึ่ง `work_group` เลย ขึ้นได้อิสระไม่ต้องรอ P1.1 เสร็จ (2) "การ์ดทีม OT อยู่ดึก" (ดูด้านล่าง) ต้องพึ่งตาราง `work_group` จาก P1.1 ก่อนถึงจะขึ้นได้ — ถ้าจะ sequence งาน แนะนำทำชั้น (1) ก่อน แล้วค่อยเพิ่มชั้น (2) หลัง P1.1 เสร็จ

**กติกาที่ยืนยันแล้ว:**

1. **ไม่บังคับ ไม่มีบทลงโทษถ้าไม่ทำ** — เป้าหมายคือให้รางวัล/เครดิตคนที่อยู่เย็น-ดึกเพิ่มขึ้นเท่านั้น ไม่กระทบสีสถานะ/badge ใดๆ ของเช้าเลย
2. **ความเข้มงวดเท่าเช้า** (ถ่ายรูป + GPS + หมายเหตุ — หมายเหตุไม่บังคับความยาวขั้นต่ำเหมือนเช้า) **แต่ไม่บล็อกการส่งแม้ GPS ไกล** — ยังคำนวณ+เก็บ `distance_m` ไว้เงียบๆ ไม่โชว์เตือน/สีม่วง เผื่ออนาคตอยากกรอง "เช็คเอาท์ที่น่าเชื่อถือ" ตอนเอาไปคำนวณรางวัลจริง
3. **เช็คเอาท์ได้เฉพาะถ้าเช็กอินเช้าของวันนั้นแล้วเท่านั้น** — เป็น hard block (ไม่ใช่แค่เตือน) ถ้ายังไม่เช็กอิน กดเช็คเอาท์ไม่ได้เลย
4. **กดเช็คเอาท์ได้หลายครั้งต่อวัน ครั้งล่าสุดชนะเสมอ** (upsert บน `(officer_id, checkout_date)`) — เผื่อกรณีกดตอน 18:00 แล้วโดนเรียกให้อยู่ต่อถึง 20:00 ก็กดใหม่ได้เลย ไม่ต้องมีกลไกแก้ไข/ลบแยก
5. **เวลาตัดสิน "ถือว่าอยู่ดึก" มีหลายระดับ (เริ่มต้น 2 tier: หลัง 18:00 / หลัง 20:00) ตั้งค่าเพิ่ม/แก้ไขได้ภายหลังโดยไม่ต้อง migration ใหม่** — ใช้ตารางแยก (ไม่ใช้คอลัมน์ตายตัว) ตามแพทเทิร์นเดียวกับ `stats_period_override` ที่โปรเจกต์นี้ใช้อยู่แล้วสำหรับของที่ "อาจเพิ่มจำนวนทีหลัง"
6. **แยกขาดจาก Grace Card สนิท ไม่เชื่อมกัน** — Grace Card ดูแค่ "มาสาย", เช็คเอาท์ดูแค่ "อยู่ถึงกี่โมง" ไม่ผสมตรรกะกัน
7. **ไม่แก้ badge ทีม OT (P1.1) ที่ยืนยันดีไซน์ไปแล้ว** — badge เดิมวัด coverage ตอนเช้า เอาข้อมูลเช็คเอาท์มาผสมจะเปลี่ยนความหมายของฟีเจอร์ที่ปิดดีไซน์ไปแล้ว ให้เป็น indicator ใหม่แยกต่างหาก (การ์ด "🌙 ใครอยู่ดึกวันนี้" คนละการ์ดกับ badge ทีม)

**สถาปัตยกรรมที่แนะนำ:**

```sql
-- บันทึกเช็คเอาท์ต่อคนต่อวัน (upsert — ครั้งล่าสุดชนะเสมอ)
create table public.check_out (
  id              uuid primary key default gen_random_uuid(),
  officer_id      uuid not null references public.officer(id),
  checkout_date   date not null,
  checked_out_at  timestamptz not null default now(),
  photo_path      text,
  lat             double precision,
  lng             double precision,
  distance_m      numeric,              -- เก็บไว้เงียบๆ ไม่บล็อก/ไม่โชว์เตือนเหมือนเช้า
  note            text,
  tier_reached    smallint,             -- snapshot ผลลัพธ์ ณ ตอนเช็คเอาท์ (ดูหมายเหตุด้านล่าง)
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now(),
  unique (officer_id, checkout_date)
);

-- ระดับเวลา "ถือว่าอยู่ดึก" แบบขยายได้ ไม่ตายตัวเป็นคอลัมน์ (ตั้งค่า/เพิ่ม/แก้ผ่าน UI ทีหลังได้)
create table public.checkout_recognition_tier (
  id              uuid primary key default gen_random_uuid(),
  tier_order      smallint not null unique,   -- 1, 2, 3, ... เรียงจากเช้าไปดึก
  threshold_time  time not null unique,        -- เช่น '18:00', '20:00'
  label           text not null,               -- เช่น "อยู่เย็น", "อยู่ดึก" ตั้งเองได้
  emoji           text not null default '🌙',
  active          boolean not null default true
);
-- seed เริ่มต้น: (1, '18:00', 'อยู่เย็น', '🌙'), (2, '20:00', 'อยู่ดึก', '🌙🌙')
```

**หมายเหตุ tier_reached (snapshot vs dynamic):** แนะนำให้ snapshot ผล tier ที่ทำได้ ณ ตอนเช็คเอาท์ลงในแถว `check_out` เอง (ไม่ใช่คำนวณสดจาก `checkout_recognition_tier` ทุกครั้งที่อ่าน) เพื่อให้สถิติย้อนหลังไม่เปลี่ยนถ้าเจ้าของมาแก้เวลา threshold ทีหลัง (สอดคล้องกับแนวทาง audit trail ที่โปรเจกต์นี้ใช้อยู่แล้วในหลายจุด เช่น `override_reason`, `retention_hold`) — เป็นข้อเสนอ ยังไม่ใช่ข้อสรุปตายตัว ถ้าเจ้าของอยากได้แบบคำนวณสดแทนก็ปรับได้ตอนเริ่มเขียนโค้ดจริง

**RPC ที่ต้องมี (ร่างชื่อ ยังไม่ใช่ signature สุดท้าย):**
- `do_check_out(p_officer_id, p_pin, p_photo_path, p_lat, p_lng, p_note)` — เช็กก่อนว่ามีแถว `check_in` ของวันนี้หรือยัง ถ้ายังไม่มี return error `not_checked_in_yet` (hard block ตามกติกาข้อ 3) ถ้ามีแล้ว upsert ลง `check_out` (คำนวณ `distance_m` + `tier_reached` ในตัว)
- `do_get_my_checkout_today(p_officer_id, p_pin)` — คืนสถานะเช็คเอาท์ของวันนี้ถ้ามี (เวลา, tier) สำหรับ UI แสดง "เช็คเอาท์แล้วเมื่อ 19:32 (กดใหม่ได้ถ้าอยู่ต่อ)"
- แก้ `do_get_my_month_stats`: เพิ่มนับจำนวนวันเช็คเอาท์ต่อ tier (เช่น "อยู่เย็น: 5 วัน, อยู่ดึก: 2 วัน")
- แก้ `do_supervisor_get_today`/`do_get_history` (และคู่ PIN): LEFT JOIN `check_out` เพิ่มคอลัมน์ "เช็คเอาท์เวลา" ต่อท้ายตารางเดิม — ใช้โครง `renderCheckinTable` เดิมได้เกือบทั้งหมด ไม่ต้องสร้าง UI ใหม่ทั้งชุด
- (รอ P1.1) `do_get_team_evening_status()`/เวอร์ชัน PIN คู่กัน — คืนรายชื่อ+เวลาเช็คเอาท์ของแต่ละ work_group วันนี้ สำหรับการ์ด "🌙 ใครอยู่ดึกวันนี้" (แยกจากการ์ด badge ทีม OT เดิม)
- ตั้งค่า tier: `do_get_checkout_tiers`/`do_set_checkout_tiers` (เฉพาะชวนชัย ผ่านการ์ดตั้งค่าระบบเดิม)

**Frontend ที่ต้องแก้ (ร่าง):**
- `index.html`: เพิ่มปุ่มรอง "🌙 เช็คเอาท์ตอนเย็น (ไม่บังคับ)" แยก flow จากเช็กอินเช้า ไม่ผูกเงื่อนไขเวลาของวัน (กดตอนไหนก็ได้) ถ้า `do_check_out` ตอบ `not_checked_in_yet` โชว์ข้อความมิตรภาพ "ต้องเช็กอินเช้าก่อนถึงจะเช็คเอาท์ได้"
- `dashboard.html`/`report.html`: เพิ่มคอลัมน์ "เช็คเอาท์เวลา" ในตารางเช็กอินวันนี้/ประวัติ + การ์ดใหม่ "🌙 ใครอยู่ดึกวันนี้" (หลัง P1.1 เท่านั้น) + สถิติส่วนตัวรายเดือนเพิ่มนับ tier

**เกณฑ์ผ่าน (ร่าง):** เช็คเอาท์ก่อนเช็กอิน → ได้ `not_checked_in_yet` เสมอ, กดเช็คเอาท์ซ้ำในวันเดียวหลายครั้ง → เหลือแถวเดียวค่าล่าสุด, GPS ไกลยังเช็คเอาท์ผ่านได้ (ไม่บล็อก) แต่ `distance_m` บันทึกถูกต้อง, สถิติรายเดือนนับ tier ถูกต้องตามเวลาที่ตั้งไว้

**หมายเหตุเลข migration:** ยังไม่ล็อกเลขตายตัวเช่นเดียวกับ P1.3 — เช็กเลขจริงจาก `CLAUDE.md` ตอนเริ่มเขียนโค้ดจริง

---

## 5. เฟส P2 — คะแนน/รางวัล + แผนก + สิทธิ์ (ส.ค. — หลังข้อมูลครบ 1 เดือน 3 ส.ค.)

### P2.1 วิเคราะห์ข้อมูลจริง 1 เดือนก่อนตั้งเกณฑ์ (ทำก่อน ห้ามข้าม)
ก่อนเขียน scoring ให้รันวิเคราะห์จากข้อมูล 3 ก.ค.–3 ส.ค. (ผ่าน SQL อ่านอย่างเดียว):
- distribution สี per คน per work_days (คนทำงาน 3 วัน/สัปดาห์ vs 5 วัน ต้องเทียบเป็น**อัตราส่วน** ไม่ใช่ยอดดิบ)
- อัตราม่วง (incomplete) per คน — ถ้าคนใดม่วงถี่ผิดปกติ อาจเป็นปัญหามือถือ/GPS ไม่ใช่พฤติกรรม **ห้ามให้ม่วงถ่วงคะแนนจนกว่าจะแยกสาเหตุได้**
- ผลของวันเริ่มนับ (`stats_period_override` ก.ค. เริ่ม 6 ก.ค.) ต้อง apply เหมือนสถิติส่วนบุคคล
- สรุปเป็นรายงานสั้นให้เจ้าของดูก่อนตัดสินใจ weights

### P2.2 Scoring framework (migration 27 — เดิมเขียนว่า 26 ในร่างแรก เลื่อนเพราะ 24 ถูกใช้ไปกับ cron timeout fix แล้ว ดูข้อ 8)
```sql
create table public.scoring_config (
  id int primary key default 1 check (id = 1),
  enabled boolean not null default false,      -- ปิดไว้จนเจ้าของสั่งเปิด
  weights jsonb not null default
    '{"green":3, "yellow":2, "orange":1, "red":0, "absent":-1, "incomplete":0}'::jsonb,
  normalize_by_workdays boolean not null default true
);
```
- RPC `do_get_month_scores(p_year, p_month)` (auth) / เวอร์ชัน PIN: คำนวณจาก `coalesce(override_status, status)` + absent (ตรรกะเดียวกับ `do_get_my_month_stats` — พิจารณา refactor เป็นฟังก์ชันภายในร่วมกันเพื่อไม่ให้เกณฑ์ 2 ที่ drift กัน) คืน ranking พร้อม raw counts เพื่อความโปร่งใส
- UI: การ์ด "🏆 สรุปคะแนนประจำเดือน" ใน dashboard/report แสดงเฉพาะเมื่อ `enabled=true` — **เห็นเฉพาะหัวหน้า** ใน v นี้ (ยังไม่โชว์ ranking ให้เจ้าหน้าที่เห็นกันเอง — กระดานจัดอันดับสาธารณะมีความเสี่ยงด้านขวัญกำลังใจ ให้เจ้าของตัดสินใจแยกทีหลังจากเห็นข้อมูลจริง)
- การให้รางวัลจริง = การตัดสินใจของเจ้าของนอกระบบ ระบบมีหน้าที่แค่คำนวณอย่างโปร่งใส ตรวจย้อนได้

### P2.3 ระบบแผนกเต็มรูป + สิทธิ์หัวหน้าตามแผนก (migration 28 — เดิมเขียนว่า 27 ในร่างแรก เลื่อนเพราะ 24 ถูกใช้ไปกับ cron timeout fix แล้ว ดูข้อ 8)
- ใช้ `work_group` จาก P1 เป็นแผนกได้เลย (เพิ่มกลุ่มที่เหลือจนครบทุกคนใน 19 คน) — นี่คือผลตอบแทนของการตัดสินใจใน P1
- ตาราง `supervisor_scope (supervisor_id uuid, work_group_id uuid, primary key ทั้งคู่)` — **ไม่มีแถว = เห็นทุกแผนก** (backward compatible: หัวหน้า 3 คนปัจจุบันไม่มีแถว → พฤติกรรมเหมือนเดิมเป๊ะ ไม่ต้อง migrate สิทธิ์)
- แก้ RPC อ่านข้อมูลฝั่งหัวหน้า (`*_get_today`, `*_get_history`, `*_get_absentees`, `*_get_team_coverage`) ให้ join กรองตาม scope ของผู้เรียก
- UI จัดการ scope อยู่ใน `dashboard.html` เฉพาะชวนชัย (แพทเทิร์นเดียวกับจัดการบัญชีหัวหน้า migration 22)
- **คำเตือน:** ฟีเจอร์นี้แตะ RPC จำนวนมากที่สุดในแผน — ทำเป็นงานเดี่ยว อย่าพ่วงกับงานอื่นในรอบ deploy เดียว และ verify ทุก RPC ที่แก้ด้วยแถวทดสอบทั้งเคส "มี scope" และ "ไม่มี scope"

---

## 6. เฟส P3 — Multi-office packaging (ก.ย. เป็นต้นไป)

### การตัดสินใจสถาปัตยกรรม: แยก instance ต่อสำนักงาน (แนะนำ) ไม่ใช่ multi-tenant DB เดียว

| ประเด็น | Option A: DB เดียว multi-tenant | Option B: แยก Supabase+Netlify ต่อหน่วย ✅ |
|---|---|---|
| ความเสี่ยงข้อมูลปนกัน | ต้องทำ RLS `office_id` ทุกตาราง/ทุก RPC — พลาดจุดเดียว = ข้อมูลข้ามหน่วย | เป็นศูนย์โดยโครงสร้าง (คนละ DB) |
| PDPA/ความเป็นเจ้าของข้อมูล | ข้อมูลทุกหน่วยอยู่ในบัญชี Supabase ของระยอง — หน่วยราชการอื่นมักไม่ยอมรับ | แต่ละหน่วยถือบัญชี/ข้อมูลของตัวเอง |
| ต้นทุน | รวมทุกหน่วยอาจชน free tier limit | free tier ต่อหน่วย — สเกล 19-30 คน/หน่วยสบาย |
| ต้นทุน refactor | สูงมาก (แก้ทุก RPC ทุกตาราง) | ต่ำ — โค้ดเดิมใช้ได้เกือบทั้งหมด |
| งาน maintain | อัปเดตครั้งเดียวได้ทุกหน่วย | ต้อง update หลาย instance — แก้ด้วย git template (ดูล่าง) |

### P3.1 กวาด hardcode ออกจากโค้ด — แก้ R5 (migration 29 — เดิมเขียนว่า 28 ในร่างแรก เลื่อนเพราะ 24 ถูกใช้ไปกับ cron timeout fix แล้ว ดูข้อ 8)
- เพิ่ม `settings.office_name text` (แสดงใน header ทุกหน้า + ชื่อหัวกล่องม็อตโต้ "Rayong Immigration Service" → ดึงจาก settings), `settings.incomplete_radius_m int default 50` (แก้ 50 ม. ที่ hardcode ใน `do_check_in`/`do_check_distance`)
- ไล่ grep ทั้ง 3 ไฟล์ HTML หา string เฉพาะระยอง แล้วย้ายเข้า settings หรือตัวแปร config หัวไฟล์
- ผูกเข้า `do_get_settings`/`do_set_settings` ให้แก้ผ่าน UI ได้

### P3.2 Bootstrap kit
- `migrations/00_bootstrap.sql` — รวม 01→2x เป็นสคริปต์เดียว idempotent สำหรับ**โปรเจกต์ Supabase ใหม่เปล่าๆ เท่านั้น** (ห้ามรันกับ production ระยองเด็ดขาด — ใส่ guard เช็คว่าตาราง settings ยังไม่มีก่อนรัน)
- `SETUP_GUIDE.md` — คู่มือติดตั้งสำหรับหน่วยใหม่: สร้าง Supabase project → รัน bootstrap → สร้าง bucket + Edge Functions 2 ตัว → ตั้ง secrets/cron → fork repo → ผูก Netlify → กรอก settings (พิกัด, ชื่อหน่วย, เกณฑ์เวลา) → เพิ่มรายชื่อเจ้าหน้าที่ → checklist ทดสอบรับงาน (ยึดเกณฑ์ผ่านเฟส A–C จากสเปกเดิม)
- ทดสอบ kit ด้วยการติดตั้งจริง 1 รอบบน Supabase project เปล่า (ใช้ฟรี tier ที่ 2 ขององค์กร Chuan ได้) — **นี่คือ verify จริงของเฟสนี้ ห้ามส่งมอบ kit ที่ไม่เคยติดตั้งจริง**

### P3.3 โมเดลการส่งมอบ (คำถามเปิด — เจ้าของตัดสินใจ ไม่ใช่งานเทคนิค)
ให้ฟรีแบบราชการช่วยราชการ (MOU/หนังสือราชการ) vs คิดค่าติดตั้ง/ดูแล — มีผลต่อว่าใครถือบัญชี Supabase/Netlify ของหน่วยใหม่ และใครรับผิดชอบตอนระบบล่ม แนะนำตอบคำถามนี้ให้ชัดก่อนเสนอหน่วยแรก

---

## 7. สิ่งที่ gate ไว้ (ยังไม่อยู่ในแผน — ต้องผ่านเงื่อนไขก่อน)

| ฟีเจอร์ | เงื่อนไขปลดล็อก |
|---|---|
| ฟีดแบ็กประชาชน + QR เคาน์เตอร์ | อนุมัติจากผู้บังคับบัญชาเป็นลายลักษณ์อักษร (เส้นแดงเดิม — ไม่มีข้อยกเว้น) |
| เช็กเอาต์ตอนเย็น | ยังไม่มีโจทย์ชัดว่าแก้ปัญหาอะไร — ข้อมูล "เลิกดึก/OT" ตอนนี้มาจากกล่องหมายเหตุอยู่แล้ว ถ้าเจ้าของอยากได้ ให้ตอบก่อนว่าหมายเหตุที่มีไม่พอตรงไหน แล้วค่อยออกแบบ |
| แสดง ranking คะแนนให้เจ้าหน้าที่เห็นกันเอง | ตัดสินใจหลังหัวหน้าเห็นข้อมูลคะแนนจริงอย่างน้อย 1 เดือน (P2.2 ให้หัวหน้าเห็นก่อน) |
| LINE/LIFF integration | นอกขอบเขตเช่นเดิม — bookmark เพียงพอ |

---

## 8. สรุปลำดับ migration + dependency

**⚠️ (10 ก.ค. 2569) ตารางนี้เลื่อนเลขทั้งหมดจากที่ร่างไว้ตอนแรก:** เลข `24` ที่แผนนี้จองไว้ให้ P0.3 ถูกใช้ไปกับงานแก้บั๊ก "cron timeout สั้นเกินไป" (`24_cron_timeout_fix.sql`) ซึ่งเป็นงานฉุกเฉินนอกแผนที่ทำไปก่อนแล้ว (ดู `CLAUDE.md` ข้อ 2.11/24.1) migration ถัดไปจริงจึงเริ่มที่ **25**

**⚠️ (11 ก.ค. 2569) เลื่อนอีกรอบ:** เลข `26` ที่จองไว้ให้ P1.1 ถูกใช้ไปกับ hotfix ฉุกเฉินอีกงาน — `26_fix_supervisor_pin_active_check.sql` แก้บั๊ก `check_and_count_pin` (migration 25) ที่บล็อกหัวหน้า PIN (ศุภัตรา/ผู้ช่วยแอดมิน) ไม่ให้ล็อกอินได้เลย เพราะเช็ก `active=true` ทั้งที่บัญชีหัวหน้าตั้งใจให้ `active=false` มาตั้งแต่ migration 11 (ดู CLAUDE.md ข้อ 29 สำหรับรายละเอียดเต็ม) — ตารางด้านล่างคือเลขที่ถูกต้องล่าสุด ถ้า agent ตัวถัดไปเจอเอกสารเก่ากว่านี้อ้างเลขอื่น ให้ยึดตารางนี้แทน

| Migration | เฟส | เนื้อหา | ขึ้นกับ |
|---|---|---|---|
| 24 | *(ใช้ไปแล้ว)* | cron timeout fix สำหรับ `delete-old-photos-daily` — งานฉุกเฉินนอกแผน ✅ เสร็จแล้ว 10 ก.ค. 2569 | — |
| 25 | P0.3 | PIN rate limiting | — |
| 26 | *(ใช้ไปแล้ว)* | hotfix แก้ `check_and_count_pin` บล็อกหัวหน้า PIN ล็อกอินไม่ได้ — งานฉุกเฉินนอกแผน ✅ เสร็จแล้ว 11 ก.ค. 2569 | — |
| 27 | P1.1 | work_group + team coverage + เกณฑ์เวลาทีม | ควรมี git (P0.2 ✅ เสร็จแล้ว) ก่อน |
| 28 | P2.2 | scoring_config + month scores | ข้อมูลครบเดือน + P2.1 |
| 29 | P2.3 | supervisor_scope + กรอง RPC ตาม scope | 27 (ใช้ work_group) |
| 30 | P3.1 | office_name + incomplete_radius_m + กวาด hardcode | — (ทำก่อน 29 ได้ถ้าอยากสลับ) |

กติกาเดิมยังใช้: migration รันบน Supabase ได้ทันทีไม่ต้องรอ confirm, ห้ามรัน 01–24 ซ้ำ (24 ใช้ไปแล้ว ไม่ใช่แค่ 01–23), DROP FUNCTION ก่อนเมื่อเปลี่ยน return columns, ทดสอบด้วยแถวชั่วคราวแล้วลบทิ้ง, ตาราง `settings` ต้อง backup/restore ค่าจริงเมื่อทดสอบเขียน

---

## 9. ความเสี่ยงของแผนนี้เอง (ให้เจ้าของ + agent ที่รันต่อชั่งใจ)

1. **P0.2 (git) เปลี่ยน workflow ที่ใช้มาตลอด** — ช่วงเปลี่ยนผ่านอาจสับสน แนะนำทำตอนไม่มีฟีเจอร์ค้าง deploy และ verify rollback ให้ได้ก่อนถือว่าเสร็จ ถ้าเจ้าของไม่สะดวก GitHub เลย ให้ถอยมาที่ minimum: โฟลเดอร์ `releases/` เก็บ snapshot 3 ไฟล์ทุกครั้งที่ deploy + คอมเมนต์ `<!-- vYYYYMMDD-HHMM -->` หัวไฟล์
2. **R2 (โค้ดซ้ำ 2 ไฟล์) แผนนี้จงใจยังไม่แก้** — การรวมเป็น shared JS คือ refactor ใหญ่ที่เสี่ยงพัง production โดยไม่เพิ่มฟีเจอร์ให้ผู้ใช้เลย คุ้มก็ต่อเมื่อมี git + preview URL แล้วเท่านั้น เสนอพิจารณาใหม่หลัง P1 เสร็จ ถ้าจะทำให้ทำเฉพาะ layer ฟังก์ชัน render ร่วม (เช่น `renderCheckinTable`) ไม่ใช่รวมทั้งไฟล์
3. **แผน P2 ตั้งอยู่บนสมมติฐานว่าข้อมูล 1 เดือนเพียงพอ** — ก.ค. มีวันหยุด/วันทดสอบปน ถ้าข้อมูลจริงบางเกินไป เลื่อนการเปิด scoring ได้โดยไม่กระทบส่วนอื่น (นี่คือเหตุผลที่ `scoring_config.enabled` default false)
4. **อย่ารันหลายงานพร้อมกัน** — แผนนี้มีงานเยอะ แต่กติกา "ทีละงาน ส่งตรวจ ถาม deploy" ยังศักดิ์สิทธิ์ agent ที่รับไปรันควรทำตามลำดับ ไม่ pipeline

---

*จัดทำ 6 ก.ค. 2569 — อัปเดตเอกสารนี้เมื่อเจ้าของเปลี่ยนทิศทาง และบันทึกงานที่เสร็จลง `CLAUDE.md` ตามวินัยเดิม*
*อัปเดตล่าสุด 10 ก.ค. 2569 — ปรับเลข migration ที่เลื่อน (24 ถูกใช้ไปกับ cron timeout fix นอกแผน), mark P0.1/P0.2 เสร็จแล้ว, เพิ่ม P0.5 (UX quick win จาก feedback เจ้าของ — ทำแล้ว 4 อย่างใน `index.html`, เหลือ 2 อย่างใน dashboard/report.html), บันทึกเบี่ยงเบนจริงของ P0.2 (repo เป็น public ไม่ใช่ private, push ตรง main ไม่ใช้ PR flow) — รายละเอียดเต็มทุกจุดอยู่ใน `CLAUDE.md` ข้อ 23/24/25/26*
*อัปเดตล่าสุด 11 ก.ค. 2569 — P0.5 การ์ด dashboard/report.html + mobile card layout report.html เสร็จสมบูรณ์+push แล้ว, P0.4 backup พักไว้ผูกกับ P3, เพิ่มดีไซน์ P1.3 Grace Card และ P1.4 เช็คเอาท์ตอนเย็น (ยังไม่เขียนโค้ดทั้งคู่ P1.4 ออกแบบไว้ก่อนแต่ไม่ผูกกับกำหนด P1 เดิม ทยอยขึ้นทีหลังได้), และเลื่อนเลข migration อีกรอบ (26 ถูกใช้ไปกับ hotfix แก้บั๊ก PIN rate limiting บล็อกหัวหน้า PIN — P1.1 จึงเลื่อนจาก 26 เป็น 27, ตามด้วย P2.2=28/P2.3=29/P3.1=30) — รายละเอียดเต็มอยู่ใน `CLAUDE.md` ข้อ 27/28/29*
