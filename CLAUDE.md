...
อัปเดตล่าสุด: 11 กรกฎาคม 2569 (P0.4 backup: ตรวจสอบสดจากหน้า Supabase Dashboard ยืนยันว่าแพลน Free ไม่มี backup อัตโนมัติเลย + เสนอทางเลือก (อัปเกรด Pro $25/เดือน vs DIY) ให้เจ้าของ + เจ้าของตัดสินใจ**พักงานนี้ไว้ก่อน ผูกกับเงื่อนไขขยายไปหน่วยอื่น/แผนอื่น (P3)** ดูข้อ 28 — ก่อนหน้านี้ (10 ก.ค. บ่าย): P0.3 PIN rate limiting SQL migration 25 รันจริงผ่าน service_role key + ทดสอบ E2E ผ่านหมด + แก้ frontend error mapping "pin_locked" ใน index.html/report.html เสร็จ **ยังรอ push อยู่ (ค้าง PAT จากเจ้าของ)** ดูข้อ 27 — ก่อนหน้านั้นเช้า 10 ก.ค.: ปิดงานบั๊กรูปกำพร้า + แก้ cron timeout + P0.1 เสร็จครบทั้งหมด + GPS timeout quick win + UX quick win รอบ 2 push แล้ว + บันทึกกระบวนการทำงาน/บทเรียน PAT-credit ลงข้อ 26 ดูข้อ 24/24.1/25/26/27/28)
...
**⚠️ (10 ก.ค. 2569) ยังไม่ได้ทำ .gitignore สำหรับ `.service_role_key.local.txt`** — ไฟล์นี้เก็บ service_role key จริง (สิทธิ์สูงสุด bypass RLS) ต้องเพิ่มเข้า `.gitignore` **ก่อน** commit/push ครั้งถัดไปที่แตะโฟลเดอร์นี้เสมอ ห้ามให้หลุดขึ้น GitHub (repo เป็น public) เด็ดขาด

**📋 (10 ก.ค. 2569) แผน UX ที่เหลือ (ยังไม่ได้ทำ):**
...
## 27. P0.3 PIN rate limiting (migration 25) — ✅ SQL รันจริงสำเร็จแล้ว + ทดสอบ E2E ผ่านหมด + แก้ frontend เสร็จ (10 ก.ค. 2569, รอ push)

**สถานะ: เขียน SQL เสร็จ + รันจริงบน Supabase ผ่าน service_role key สำเร็จทุกขั้นตอน + ทดสอบ E2E จริงผ่านหมด + แก้ `index.html`/`report.html` เสร็จ + verify syntax ผ่านทั้งคู่ — เหลือแค่ส่งให้เจ้าของตรวจสอบแล้วถามก่อน push (SQL ฝั่ง Supabase รันไปแล้วจริง มีผลใช้งานทันทีตามกติกาข้อ 2.1 ข้อยกเว้น แต่ไฟล์ `.html` 2 ไฟล์ยังไม่ได้ push)**
