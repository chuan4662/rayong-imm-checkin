...
**✅✅✅ (16 ก.ค. 2569) P1b (เหรียญทอง/เงิน daily_rank) เสร็จสมบูรณ์ backend + frontend + E2E test + push ขึ้น git สำเร็จแล้ว — ดูข้อ 32:** เจ้าของสั่งให้เริ่มทันทีตามแผนที่บันทึกไว้ในข้อ 7 (backlog) จากรอบก่อนหน้า — Backend: คอลัมน์ `check_in.daily_rank` + แก้ 4 ฟังก์ชัน (`do_check_in_impl`/`do_get_today_status_impl`/`do_supervisor_get_today_impl`/`do_get_my_month_stats_impl`) — **⚠️ เจอบั๊กจริงระหว่าง E2E test: `do_supervisor_get_today_impl` ที่ควรถูกแก้ไปแล้วในรอบก่อนหน้า (ก่อน compaction ของ session) จริงๆ แล้วไม่มี `daily_rank` ในคำตอบเลย — วินิจฉัยจาก `pg_get_functiondef` พบว่าการแก้ไม่เคยถูกรันจริงบน DB แก้ใหม่ทันทีในรอบนี้ verify ผ่านแล้ว บทเรียน: ห้ามเชื่อสรุปงานจากรอบก่อนหน้าเฉยๆ ต้อง verify ของจริงบน DB เสมอ (ดูกติกาใหม่ข้อ 2.15)** — Frontend: `index.html` (ข้อความยินดี 🥇🥈 + สถิติสะสมรายเดือน), `dashboard.html`/`report.html` (ไอคอนเหรียญในตารางเช็กอินวันนี้ + เปลี่ยนชื่อการ์ด "จุดบริการกลุ่มงาน OT" เป็น "แผนกงานการให้บริการ" ตามที่เจ้าของสั่ง) — E2E: ทดสอบผ่าน RPC จริงครบ 6 เคส (rank=1, rank=2, rank=null เมื่อ incomplete, rank=null เมื่อครบ 2 คนแล้ว, incomplete ไม่กินสิทธิ์อันดับ, gold/silver stats) + verify `do_supervisor_get_today`/`do_get_my_month_stats` คืนค่าถูกต้อง + ลบข้อมูลทดสอบ/officer ปลอม/ฟังก์ชันชั่วคราวออกครบ + คืนค่า settings/PIN กลับสภาพเดิม — **push สำเร็จ (commit `a98f4c7`), verify บนเว็บจริงผ่าน** (`fetch()` เช็ก marker string `MedalBox`/`medalTag`/ชื่อการ์ดใหม่ครบทั้ง 3 ไฟล์บน `rayongimm.link`)

**✅✅✅ (16 ก.ค. 2569) P1.4 (เช็คเอาต์ตอนเย็น, migration 31) เสร็จสมบูรณ์ backend + frontend (index.html/dashboard.html/report.html) + E2E test + push ขึ้น git สำเร็จแล้ว — ดูข้อ 33:** เจ้าของปรึกษาแผน P1 ที่เหลือ (เช็คเอาต์เย็น + Grace Card) แบบ Strategic Partner แล้วเลือกเริ่มเช็คเอาต์เย็นก่อน — **⚠️ เบี่ยงจากดีไซน์ร่างเดิมใน roadmap 2 จุดตามที่เจ้าของตัดสินใจ:** (1) ล็อกเช็คเอาต์ได้ครั้งเดียวเด็ดขาดต่อวัน (ไม่ใช่กดซ้ำได้ครั้งล่าสุดชนะ) (2) เกณฑ์เวลา tier 18:00/20:00 เขียนตายตัว ยังไม่มี UI แก้ไข — Backend: ตาราง `check_out`/`checkout_recognition_tier` (seed 2 tier), RPC `do_check_out`/`do_get_my_checkout_today` (thin wrapper ตามแพทเทิร์น PIN rate limiting เดิม), แก้ 4 ฟังก์ชันเดิม (`do_get_my_month_stats_impl`, `do_supervisor_get_today_impl`, `do_supervisor_get_history_impl`, `do_get_history`) ทุกตัว signature เดิมไม่ต้อง DROP — **⚠️ เจอ+แก้ช่องว่างเดิมจาก P1b ระหว่างทำ: `do_get_history` (เวอร์ชัน auth-only ที่ dashboard.html ใช้ คนละตัวกับ `do_supervisor_get_history_impl` แบบ PIN) ไม่เคยมี `daily_rank` เลยตั้งแต่ P1b เพิ่มเข้าไปพร้อมกันในรอบนี้เพราะแก้ไฟล์เดียวกันอยู่แล้ว** — Frontend: `index.html` เพิ่มปุ่ม "🌙 เช็คเอาต์ตอนเย็น" ที่หน้า login พร้อม precheck 2 ชั้นก่อนเปิดกล้อง (เช็กอินแล้วหรือยัง + เช็คเอาต์ไปแล้วหรือยัง) กันรูปกำพร้าเหมือนแพทเทิร์นเช็กอินเช้า, `dashboard.html`/`report.html` เพิ่มคอลัมน์ "🌙 เช็คเอาต์" ในตารางวันนี้/ประวัติ (dashboard.html ต้อง query `check_out` แยกแล้ว merge เองฝั่ง client เพราะตารางวันนี้ไม่ผ่าน RPC) — E2E: ทดสอบ RPC จริงครบ (not_checked_in_yet ก่อนเช็กอิน, tier1/tier2 ตามเวลาจริง, already_checked_out ซ้ำ, GPS ไกล 134 กม. ไม่บล็อก, `do_get_history`/`do_supervisor_get_history` คืน checkout fields ถูกต้อง, auth gate ของ `do_get_history` ยังทำงานปกติ) ลบข้อมูลทดสอบ+ฟังก์ชันชั่วคราวออกครบ — push สำเร็จ, verify บนเว็บจริงผ่าน


**✅✅ (20 ส.ค. 2569) migration 32 — ระบบ "เหลื่อมเวลา" ระดับกลุ่มงาน: SQL รันบน production แล้ว + E2E 13 เคสผ่านหมด (ทดสอบแบบ rollback ไม่แตะข้อมูลจริง) + แก้ frontend ครบ 3 ไฟล์ — ⚠️ ยังไม่ push รอเจ้าของ confirm ดูข้อ 38:** งานครอบครัว/งานธุรกิจ "ชุดแรก" ต้องเช็กอินทันก่อน 08:30 แล้วคนที่เหลือของกลุ่มได้เกณฑ์สี +30 นาที พร้อมป้าย `⏱ เหลื่อมเวลา` — **deploy แล้วยังไม่มีอะไรเปลี่ยนทันที เพราะทุกกลุ่มตั้ง `stagger_enabled=false` ไว้ หัวหน้าต้องเปิดสวิตช์เองในหน้า dashboard** (และควรแจ้งเจ้าหน้าที่ทุกคนก่อน — ประเด็นความเป็นธรรม ดูข้อ 38.6)
อัปเดตล่าสุด: **20 สิงหาคม 2569** (migration 32 ระบบเหลื่อมเวลาระดับกลุ่มงาน — backend+frontend+E2E เสร็จ ยังไม่ push ดูข้อ 38) — ก่อนหน้านี้ 22 กรกฎาคม 2569 (✅ แก้บั๊ก "ค้นประวัติช่วงยาวใน report.html แล้วเว็บค้าง" — lazy load รูป + จำกัดคิว 5 รูป commit `11ff738` ดูข้อ 37 · 📝 บันทึกดีไซน์ "ระบบจองคิวออนไลน์ ตม.ระยอง" ลง backlog ข้อ 7 (ยังไม่เริ่มเขียนโค้ด ห้าม agent เริ่มเองจนกว่าเจ้าของสั่ง) — ก่อนหน้านั้นวันเดียวกัน: ✅ deploy เฟส C — แท็บ "เสียงประชาชน" (ติ-ชม) + กล่องคำชมจากประชาชนหลังเช็กอิน push ขึ้น production แล้ว commit `6cae913` ดูข้อ 36 — ก่อนหน้านั้นวันเดียวกัน: กู้แบนเนอร์น้องริมมี่ที่เอเจนต์อีกตัว deploy แบบ manual แล้วโดน git push ทับหาย กลับมาเป็นโค้ด/รูปต้นฉบับ commit `1021481` ดูข้อ 35 + ลบปุ่มลัดหมายเหตุ commit `016e6a5` — ก่อนหน้านี้ 19 ก.ค.: ลบปุ่มลัดหมายเหตุ "ปฏิบัติงานปกติ" ออกจาก index.html ตามคำสั่งเจ้าของ + บันทึกกติกาใหม่เรื่องบัญชีทดสอบ "สมาชิกใหม่" push แล้ว ดูข้อ 34 — ก่อนหน้านี้ 16 ก.ค.: P1.4 เสร็จสมบูรณ์ backend+frontend+E2E+push ครบแล้ว ดูข้อ 33 — ก่อนหน้านี้วันเดียวกัน: P1b เสร็จสมบูรณ์ backend+frontend+E2E+push ครบแล้ว ดูข้อ 32 — ก่อนหน้านี้ 13 ก.ค.: P1a + migration 29 เสร็จสมบูรณ์ทั้งคู่ push ขึ้น git รวมกันในรอบนี้แล้ว ดูข้อ 30/31 — ก่อนหน้านี้ 11 ก.ค.) (พบ+แก้บั๊กจริงบน production: ศุภัตราเข้า report.html ไม่ได้ เพราะ `check_and_count_pin` migration 25 บล็อกหัวหน้า PIN ทุกคน — วินิจฉัยจริงผ่าน SQL Editor, แก้เป็น migration 26, verify ผ่าน REST จริง, เจ้าของยืนยันเข้าได้ปกติแล้ว, push ขึ้น git แล้ว (commit `310f123`) ดูข้อ 29 — ก่อนหน้านั้นวันเดียวกัน: P0.5 จัดกลุ่มการ์ด dashboard/report.html + mobile card layout report.html เสร็จสมบูรณ์+push แล้ว (commit `5d146fd`, `310f123`), เพิ่มดีไซน์ P1.3 Grace Card ใน roadmap (ยังไม่เขียนโค้ด), P0.4 backup ตรวจสอบสดแล้วเจ้าของสั่งพักไว้ก่อนผูกกับ P3 ดูข้อ 28 — ก่อนหน้านี้ (10 ก.ค. บ่าย): P0.3 PIN rate limiting SQL migration 25 รันจริง+ทดสอบ E2E+แก้ frontend+push ขึ้น git สำเร็จครบ (commit `4ccfa23`) ดูข้อ 27 — ก่อนหน้านั้นเช้า 10 ก.ค.: ปิดงานบั๊กรูปกำพร้า + แก้ cron timeout + P0.1 เสร็จครบทั้งหมด + GPS timeout quick win + UX quick win รอบ 2 push แล้ว + บันทึกกระบวนการทำงาน/บทเรียน PAT-credit ลงข้อ 26 ดูข้อ 24/24.1/25/26/27/28/29)
...
- **เช็คเอาต์ตอนเย็น ไม่บังคับ (P1.4)** — ดูข้อ 33 ด้านล่าง
...
**⚠️ (16 ก.ค. 2569 เย็น) เปลี่ยนกติกาการทดสอบด้วยบัญชี "สมาชิกใหม่" — เจ้าของสั่งให้เลิกลบ/รีเซ็ต PIN ของบัญชีนี้หลังทดสอบเสร็จแล้ว:** ที่มา: เจ้าของล็อกอินด้วย "สมาชิกใหม่" เองจริงจัง (ทดสอบเช็คเอาต์เย็น) แล้วสงสัยว่าทำไมเห็น "ขาดเช็กอิน 8 ครั้ง" ทั้งที่รวมเช็กอินเดือนนี้แค่ 1 ครั้ง กับทำไมต้องตั้ง PIN ใหม่ทุกครั้ง — ตรวจสอบแล้วพบว่าเป็นผลจากกติกาเดิม (agent ลบแถว check_in ทดสอบ + รีเซ็ต `pin_hash=null` กลับหลังทดสอบเสร็จทุกรอบ ตามที่บันทึกไว้ในเกือบทุก migration section 30-33) ทำให้บัญชีนี้ไม่มีประวัติเช็กอินจริงสะสมเลยและต้องตั้ง PIN ใหม่ทุกครั้งที่มีคนมาลองระบบจริง — **เจ้าของยืนยันว่าไม่ใช่บั๊ก แต่ไม่ต้องการพฤติกรรมแบบนี้อีกต่อไป: "ไม่ต้องลบ หลังสมาชิกใหม่ ทดสอบเสร็จ สามารถปนกับของจริงได้ ไม่ต้องรีเซ็ทพิน"** — **กติกาใหม่ตั้งแต่นี้ไป: agent ที่ทดสอบด้วยบัญชี "สมาชิกใหม่" (officer ทั่วไป `active=true`, `id=bcfe3d7c-ac87-4d6e-b684-a0072c1b9614`) ไม่ต้องลบแถว `check_in` ทดสอบทิ้งหลังเสร็จอีกต่อไป และไม่ต้องรีเซ็ต `pin_hash`/`pin_fail_count`/`pin_locked_until` กลับเป็นค่าว่างอีกต่อไป ปล่อยให้ข้อมูลทดสอบปนกับของจริงได้เลย** — ⚠️ กติกานี้ใช้เฉพาะบัญชี "สมาชิกใหม่" เท่านั้น **บัญชีทดสอบหัวหน้า PIN "สมาชิกใหม่ที่ขยันขันแข็ง" (ข้อ 12 ก.ค. ด้านบน) ยังคงต้องลบแถวทดสอบ+รีเซ็ต PIN กลับตามเดิมทุกครั้งเหมือนเดิม** ไม่เปลี่ยน (เพราะเป็นบัญชีหัวหน้าที่ใช้ทดสอบ RPC `do_supervisor_*` ซ้ำๆ หลายรอบ ถ้าปล่อยข้อมูลปนจะกระทบการทดสอบ badge ทีม/coverage ในอนาคต) — **ยืนยันแล้วว่า PIN reset ไม่เคยลบ/กระทบประวัติเช็กอิน (`check_in`) เลยไม่ว่าใครก็ตาม** (RPC `do_reset_pin`/`do_supervisor_reset_pin_impl` แก้แค่คอลัมน์ `pin_hash`/`pin_fail_count`/`pin_locked_until` ของ `officer` เท่านั้น ไม่แตะตาราง `check_in` แม้แต่บรรทัดเดียว) เจ้าหน้าที่จริงที่เคยลืม PIN แล้วโดนหัวหน้ารีเซ็ตให้ ข้อมูลเช็กอินเก่าไม่เคยหายแม้แต่คนเดียว
...
15. **⚠️ (ใหม่ 16 ก.ค. 2569) ห้ามเชื่อว่า "งานเสร็จแล้ว" จากสรุปงานของรอบ/เซสชันก่อนหน้าเฉยๆ (รวมถึง compacted summary ของเซสชันตัวเอง) โดยไม่ verify ของจริงบน DB ก่อนเสมอ** — เจอจริงระหว่างทำ P1b (migration 30, ข้อ 32): สรุปงานจากก่อน compaction อ้างว่าแก้ `do_supervisor_get_today_impl` ให้คืน `daily_rank` แล้วเสร็จสมบูรณ์ แต่ตอน E2E test จริง (เรียก RPC ตรงๆ) พบว่าคำตอบไม่มี `daily_rank` เลย ตรวจสอบ source จริงผ่าน `pg_get_functiondef` ยืนยันว่าการแก้นั้น**ไม่เคยถูกรันจริงบน DB** (น่าจะเป็นเพราะ Monaco editor corruption/CDP timeout ที่เกิดขึ้นบ่อยในเซสชันที่พิมพ์ SQL ยาวๆ ผ่านเบราว์เซอร์ ดูข้อ 30.4/30.5) — **บทเรียน:** ทุกครั้งที่ agent ตัวใหม่รับงานต่อ (โดยเฉพาะหลัง context compaction) ต้อง treat สรุปงานเป็น "สิ่งที่ตั้งใจทำ" ไม่ใช่ "สิ่งที่ยืนยันแล้วว่าทำสำเร็จ" — ก่อนใช้ RPC ใดที่สรุปงานอ้างว่าแก้ไปแล้ว ให้เรียก RPC จริงหรือ query `pg_get_functiondef`/`pg_proc` เพื่อยืนยัน behavior ปัจจุบันตรงกับที่สรุปไว้ก่อนเสมอ โดยเฉพาะถ้าจะเอาไปอ้างอิงต่อ (เช่น เขียนเอกสาร migration, บอกเจ้าของว่า "เสร็จแล้ว")
16. **✅ (ใหม่ 20 ส.ค. 2569) วิธีรัน SQL ที่เชื่อถือได้กว่าการพิมพ์ใน Monaco — ยิงผ่าน pg-meta API ด้วย session ของหน้า SQL Editor เอง** — บั๊ก Monaco (focus-loss, auto-close วงเล็บ, `type` timeout, การแก้ไม่ถูกรันจริงตามข้อ 2.15) ทำให้เสียเวลาทุกรอบที่ผ่านมา รอบนี้ใช้วิธีใหม่ที่ได้ผล 100%: เปิดหน้า SQL Editor ปกติ → hook `window.fetch` เก็บ header ของคำขอที่ปุ่ม Run ยิงจริง (กด Ctrl+Enter หนึ่งครั้งด้วย query สั้นๆ) → แล้วเรียก `POST https://api.supabase.com/platform/pg-meta/<ref>/query?key=` ด้วย header ชุดเดียวกันผ่าน `javascript_tool` ⇒ ได้ผลลัพธ์กลับมาเป็น **JSON ตรงๆ** ไม่ต้องอ่านจากตารางผลลัพธ์ (ซึ่งอ่านไม่ได้เพราะ virtualized) และไม่มีปัญหาเนื้อหาเพี้ยนจากการพิมพ์เลย — **ข้อควรระวัง:** (1) ส่งทีละบล็อกย่อย บล็อกใหญ่/มี `DROP FUNCTION` ปนหลายคำสั่งอาจโดน safeguard ตัด ให้แยกคำสั่ง `DROP` ออกมารันเดี่ยวๆ แล้ว **CREATE ตามทันที** อย่าทิ้งช่วง (ระหว่างนั้นฟังก์ชันหายไปจากระบบจริง) (2) การดึง `pg_get_functiondef` ก้อนใหญ่กลับมาอ่านมักโดน safeguard บล็อก ให้ใช้วิธี **สร้าง Blob แล้ว `a.click()` ดาวน์โหลดเป็นไฟล์** แล้วอ่านจากโฟลเดอร์ Downloads แทน (3) token ในหน้ามีอายุจำกัด ถ้าโดน 401 ให้กด Run ซ้ำหนึ่งครั้งเพื่อเก็บ header ใหม่
17. **✅ (ใหม่ 20 ส.ค. 2569) E2E test ที่แตะ production ให้ห่อด้วย `begin; ... rollback;` เสมอ ถ้าทำได้** — รอบ migration 32 ทดสอบครบ 13 เคส (สร้างกลุ่มทดสอบ, สร้าง officer ทดสอบ, เรียก `do_check_in` จริง, ตรวจผลทุกเคส) ภายในทรานแซกชันเดียวแล้ว `rollback` ⇒ **production ไม่มีร่องรอยตกค้างเลยแม้แต่แถวเดียว ไม่ต้องมีขั้นตอน cleanup ที่อาจลืม** (verify หลังทดสอบว่าไม่มี officer/group ทดสอบเหลือ, ค่า settings กลับเป็นเดิม, จำนวนแถว `check_in` เท่าเดิม) — โบนัส: `now()` คงที่ตลอดทรานแซกชัน จึงจำลอง "มาสายกี่โมง" ได้ด้วยการขยับเกณฑ์เวลาชั่วคราวแทนการรอเวลาจริง ทำให้ทดสอบครบทุกแถบสีได้ในไม่กี่วินาที
...
| `30_daily_rank_medals.sql` | **P1b: เหรียญทอง/เงิน เช็กอินคนแรก-คนที่สองของวัน (daily_rank)** — เพิ่ม `check_in.daily_rank` (smallint, 1/2/null), แก้ 4 ฟังก์ชัน (`do_check_in_impl` คำนวณอันดับตอนเช็กอิน, `do_get_today_status_impl`/`do_supervisor_get_today_impl`/`do_get_my_month_stats_impl` คืนค่าเพิ่ม) ทุกฟังก์ชัน signature เดิมไม่เปลี่ยน ไม่ต้อง DROP ก่อน — ดูข้อ 32 |
| `31_evening_checkout.sql` | **P1.4: เช็คเอาต์ตอนเย็น (ไม่บังคับ)** — ตาราง `check_out` (unique ต่อคนต่อวัน ล็อกครั้งเดียว) + `checkout_recognition_tier` (seed 2 tier: 18:00 อยู่เย็น, 20:00 อยู่ดึก), RPC `do_check_out`/`do_get_my_checkout_today` (thin wrapper ตามแพทเทิร์น PIN rate limiting), แก้ 4 ฟังก์ชันเดิม (`do_get_my_month_stats_impl`, `do_supervisor_get_today_impl`, `do_supervisor_get_history_impl`, `do_get_history`) เพิ่ม LEFT JOIN check_out ทุกตัว signature เดิมไม่เปลี่ยน ไม่ต้อง DROP ก่อน — ดูข้อ 33 |
| `32_staggered_shift.sql` | **เหลื่อมเวลาระดับกลุ่มงาน (Staggered Shift)** — `work_group.stagger_enabled`/`core_open_before`(08:30)/`core_min_count`(1), `settings.stagger_minutes`(30), `check_in.stagger_applied`; แก้ `do_check_in_impl` ให้เลื่อนเกณฑ์สีให้ "ชุดหลัง" เมื่อมีคนอื่นในกลุ่มเช็กอินสมบูรณ์ทันก่อนเส้นตาย; RPC อ่านข้อมูล 5 ตัวคืน `stagger_applied`/`stagger_count` เพิ่ม (signature เดิม); DROP+recreate `do_admin_list_work_groups`/`do_admin_update_work_group`/`do_set_settings` เพราะเปลี่ยน parameter list — ดูข้อ 38 |

**⚠️ หมายเหตุเลข migration ถัดไป:** เลข 24 ถูกใช้ไปกับ cron timeout fix, เลข 26 ถูกใช้ไปกับ hotfix ฉุกเฉิน, เลข 27–28 ใช้ไปกับ P1a (work_group + RPC จัดการกลุ่ม), เลข 29 ใช้ไปกับการห้ามเช็กอินก่อนเวลา, เลข 30 ใช้ไปกับ P1b (เหรียญทอง/เงิน), เลข **31 ใช้ไปกับ P1.4 (เช็คเอาต์ตอนเย็น) แล้ว** (16 ก.ค. 2569 — ดูข้อ 33) เลข **32 ใช้ไปกับระบบเหลื่อมเวลาระดับกลุ่มงานแล้ว** (20 ส.ค. 2569 — ดูข้อ 38) migration ถัดไปจริงคือ **33** — ดูตารางเต็มใน `90_ROADMAP_v2_PLAN.md` ข้อ 8
...
**ปิดงานแล้ว (16 ก.ค. 2569):** เหรียญทอง/เงิน เช็กอินคนแรก-คนที่สองของวัน (P1b, migration 30) — backend+frontend+E2E test+push ขึ้น git สำเร็จครบทั้งหมด ดูข้อ 32

**ปิดงานแล้ว (16 ก.ค. 2569):** เช็คเอาต์ตอนเย็น ไม่บังคับ (P1.4, migration 31) — backend+frontend+E2E test+push ขึ้น git สำเร็จครบทั้งหมด ดูข้อ 33 — **Grace Card (P1.3) ยังไม่เริ่ม รอเจ้าของตัดสินใจเรื่องการจัดกลุ่มเจ้าหน้าที่ที่เหลือ (อัปเดต 20 ส.ค. 2569: เหลือ **4 คน** ที่ยังไม่มี work_group — จอย/เม่/ปาย/แพร) ก่อน**

**แผนใหม่ที่ยืนยันดีไซน์แล้ว แต่ยังไม่เริ่มเขียนโค้ด (บันทึก 13 ก.ค. 2569 — เจ้าของบอกว่า "เก็บเป็น backlog ก่อน" ยังไม่ให้เริ่ม):**
- **ตัดปุ่ม "สลับกล้อง (หน้า/หลัง)" ออกจากหน้าเช็กอิน `index.html`** — ที่มา: เจ้าของสังเกตว่ามีเจ้าหน้าที่บางคน (ไม่ใช่หลายคน) ไม่ยอมเซลฟี่ตัวเอง ถ่ายรูปสถานที่แทน อาจเพราะอายหรือยังไม่พร้อม ตรวจสอบโค้ดจริงพบว่าต้นตอคือปุ่ม "สลับกล้อง (หน้า/หลัง)" ที่มีอยู่แล้ว (บรรทัด ~159, `openCamera(facingMode)`) เปิดช่องให้สลับไปกล้องหลังแล้วถ่ายรูปอื่นแทนตัวเองได้ตลอด — **ที่ปรึกษากันแล้วแนะนำไม่ให้ทำ face detection/face recognition** เพราะ (1) face detection ตรวจแค่ "มีวัตถุคล้ายใบหน้าไหม" ไม่พิสูจน์ตัวตนจริง ยังถือรูปคนอื่นมาหลอกได้ (2) face recognition (เทียบใบหน้าจริง) เป็นข้อมูล biometric อ่อนไหวสำหรับหน่วยงานราชการ ควรอยู่ในระดับเดียวกับ "ฟีดแบ็กประชาชน + QR" ที่ต้องรออนุมัติเป็นลายลักษณ์อักษรจากผู้บังคับบัญชาก่อน ไม่ใช่สิ่งที่ควรตัดสินใจทำเองระดับเทคนิค (3) ถ้าตรวจพลาด (แสงน้อยตอนเช้ามืด, หมวก/เครื่องแบบบังหน้า) แล้วบล็อกไม่ให้ส่ง จะกระทบสีเช็กอิน (เขียว/เหลือง/ส้ม/แดง) ของคนที่เช็กอินจริงและตรงเวลาอย่างไม่เป็นธรรม — **ดีไซน์ที่ตกลงกันแล้ว (ยังไม่เขียนโค้ด):** ตัดปุ่ม "สลับกล้อง (หน้า/หลัง)" ออก บังคับกล้องหน้าอย่างเดียว + เสริมข้อความให้กำลังใจใกล้ปุ่มถ่ายรูป เช่น "ไม่ต้องแต่งหน้าก็ได้ครับ แค่ให้เห็นหน้าชัดๆ เพื่อยืนยันตัวตน" ต้นทุนแทบเป็นศูนย์ ไม่มีความเสี่ยง false reject เพราะไม่มีการตรวจจับอัตโนมัติใดๆ เลย — เนื่องจากปัญหานี้เกิดกับคนแค่ไม่กี่คน เจ้าของอาจแก้ด้วยการพูดคุยตักเตือนตรงจากหัวหน้าก่อนก็ได้โดยไม่ต้องรอ deploy โค้ดเลย — **⚠️ agent ตัวถัดไป: ห้ามเริ่มเขียนโค้ดเองจนกว่าเจ้าของจะสั่งให้เริ่ม**

**แผนใหม่ที่ปรึกษากันแล้ว แต่ยังไม่ตัดสินใจดีไซน์สุดท้าย (บันทึก 16 ก.ค. 2569 — เจ้าของบอกว่า "ยังไม่ต้องเขียนโค้ด แค่บันทึกการวางแผนไว้ก่อน"):**
- **สถิติเช็คเอาต์เย็นส่วนตัวรายเดือน แสดงในการ์ดสถิติหลังเช็กอิน** — ที่มา: เจ้าของถามว่าควรให้แต่ละคนเห็นสถิติของตัวเองไหมว่าเช็คเอาต์เย็นไปกี่ครั้งในแต่ละเดือน หลังเช็กอินเช้าเสร็จ (จุดเดียวกับการ์ดสถิติส่วนตัวที่มีอยู่แล้ว — สีเขียว/เหลือง/ส้ม/แดง/ขาดเช็กอิน/เหรียญทอง-เงิน) — **ต้นทุนต่ำมาก เพราะ backend มีข้อมูลพร้อมอยู่แล้ว:** ตอนทำ P1.4 (migration 31, ดูข้อ 33) ใส่ `checkout_tiers` (array แยกนับตาม tier อยู่เย็น/อยู่ดึก) กับ `checkout_total` ไว้ใน `do_get_my_month_stats_impl` แล้ว แค่ยังไม่ได้เอามาโชว์ฝั่ง `index.html` เท่านั้น (`loadMyStats()`) — เป็นงาน frontend ล้วนๆ ไม่ต้องแก้ backend เพิ่ม
  - **คำถามที่คุยกันไว้ แต่ยังไม่ได้ข้อสรุป (เจ้าของขอพักไว้ก่อน ยังไม่ให้เริ่มเขียนโค้ด):**
    1. **ตำแหน่งที่แสดง** — ต่อท้ายการ์ดสถิติเดิมหลังเช็กอินเช้าอย่างเดียว หรือโชว์คู่กัน 2 จุด (เพิ่มอีกจุดที่หลังเช็คเอาต์เย็นเสร็จ `showCheckoutResult` ด้วย เพื่อ feedback ทันทีตอนกระทำจริง)
    2. **ระดับรายละเอียด** — ยอดรวมอย่างเดียว ("เช็คเอาต์แล้ว 5 ครั้ง") หรือแยกตาม tier ด้วย ("อยู่เย็น 3, อยู่ดึก 2") โดยใช้ข้อมูล `checkout_tiers` ที่มีอยู่แล้วเต็มที่
    3. **กรณี `checkout_total = 0`** — ซ่อนแถวนี้ไปเลยเงียบๆ (ข้อเสนอของผม เพราะเช็คเอาต์เย็นเป็นฟีเจอร์ไม่บังคับ ไม่อยากให้คนที่ไม่เคยทำรู้สึกเหมือนถูกจับตา/ถูกคาดหวัง) หรือโชว์ข้อความส่งเสริมแบบเบาๆ แทนตัวเลข 0
  - **ข้อควรระวังเชิงดีไซน์ที่ควรยึดไว้เวลาตัดสินใจจริง:** ต้อง frame เป็น "โบนัส/คำชม" ไม่ใช่ "ตัวชี้วัด" ให้ต่างจากสถิติสีน้ำตาล "ขาดเช็กอิน" ที่เป็น compliance metric เพราะเช็คเอาต์เย็นสมัครใจล้วนๆ
  - **⚠️ agent ตัวถัดไป: ห้ามเริ่มเขียนโค้ดฟีเจอร์นี้เองจนกว่าเจ้าของจะตอบ 3 คำถามข้างต้นแล้วสั่งให้เริ่ม**

**แผนใหม่: ระบบจองคิวออนไลน์ ตม.ระยอง — ดีไซน์นิ่งแล้ว ยังไม่เริ่มเขียนโค้ด (ปรึกษาแบบ Strategic Partner + เจ้าของ confirm ครบทุกข้อ 22 ก.ค. 2569):**
- **แนวคิด:** เว็บจองคิวออนไลน์ (static site แยก เหมือนเว็บฟีดแบก) แทรกลิงก์ในริชเมนูไลน์หน่วยงานน้องริมมี่ (ขวาล่าง) — ผู้ใช้บริการ (คตด.) จองคิวล่วงหน้า ระบบส่งคิวถึง "ตัวเจ้าหน้าที่รายคน" ให้เห็น/จัดการในพื้นที่ส่วนตัวหลังเช็กอิน (จุดเดียวกับกล่องคำชม/สถิติ)
- **การตัดสินใจดีไซน์ที่เจ้าของ confirm แล้ว:**
  1. **คิวถึงตัวคน ไม่ใช่กองกลางกลุ่มงาน** — ระบบแจกอัตโนมัติแบบ **round-robin** เฉพาะ จนท. ที่หัวหน้าติ๊ก "รับคิวออนไลน์" ไว้และวันนั้นยังไม่เต็มโควตา (คนจองไม่เห็น/ไม่ได้เลือกชื่อ จนท.) — หัวหน้ามีสิทธิ์กำหนดรายคนว่าใครไม่ต้องรับคิวออนไลน์ (บางคนมีภาระงานอื่นนอกจากหน้าเคาน์เตอร์) — งานธุรกิจ/ครอบครัวในอนาคตค่อยใช้โมเดลกองกลางกลุ่มงาน (`work_group` ที่มีอยู่แล้ว)
  2. **โควตาระยะแรก: 1 คิว/คน/วัน** (ลดจากไอเดียแรกเช้า 1 บ่าย 1 — เริ่มเล็ก)
  3. **ประเภทงานระยะแรก: One Stop Service เท่านั้น** (หลายเคาน์เตอร์ เอกสารไม่ซับซ้อน เช็คลิสต์เอาอยู่) — งานธุรกิจ/ครอบครัวเอกสารซับซ้อนกว่า รอดูสักระยะค่อยพัฒนาต่อ
  4. **48 ชม. ไม่กดรับ → ไม่ auto-forward เด็ดขาด** (เจ้าของชี้ว่าถ้าส่งต่ออัตโนมัติ จนท. จะแกล้งมึนไม่กดรับ = ยิ่งเมินยิ่งสบาย) — ใช้กลไกความรับผิดชอบแทน: (ก) การ์ดคิวค้างโชว์ทุกครั้งหลังเช็กอิน เหลือง→แดงเมื่อเกิน 48 ชม.ทำการ อ้างไม่เห็นไม่ได้ (ข) เกิน 48 ชม. เด้งแดชบอร์ดหัวหน้าพร้อมชื่อคนค้าง หัวหน้าเป็นคนโยกเอง ไม่ใช่ระบบ (ค) สถิติรับช้า/ค้างสะสมรายเดือนต่อคน (แรงจูงใจแบบเดียวกับสีเช็กอิน ไม่มีบทลงโทษในระบบ) (ง) ฝั่งผู้จองเห็นสถานะ "อยู่ระหว่างตรวจสอบ" เสมอ ไม่ถูกทิ้งเงียบ
  5. **ภาษา: ฝั่งผู้จอง 2 ภาษา (ไทย+อังกฤษ) / ฝั่ง จนท. ไทยล้วน** — ทำได้เพราะบังคับข้อมูลเป็น structured เกือบหมด (เช็คลิสต์/ประเภทงาน/template เก็บ 2 ภาษาใน DB ผู้จองเห็น EN จนท. เห็น TH ของข้อความเดียวกัน) — **คำถาม 1 ครั้งของ จนท. เลือกจาก template 2 ภาษา ~5-6 คำถามยอดฮิต** (เช่น "แจ้งที่พัก TM30 แล้วหรือยัง") จนท. กดเลือกเป็นไทย ผู้จองเห็นเป็นอังกฤษอัตโนมัติ คำตอบผู้จอง = ใช่/ไม่ใช่ + ช่องข้อความสั้น optional
  6. **Flow:** จองล่วงหน้า ≥10 วัน (ปรับจาก 7 ให้มี buffer) → ติ๊กเช็คลิสต์ครบตามประเภทวีซ่า + ยืนยัน "เตรียมเอกสารครบ" → ยืนยันอีเมลผ่านลิงก์ก่อนคิวเข้าระบบ (กันจองเล่น) → ระบบ round-robin แจก → จนท. กดรับใน 48 ชม.ทำการ → ถามได้ 1 คำถาม (ตอบผ่านหน้าเว็บจากลิงก์ในอีเมล ไม่ใช่ตอบอีเมลดิบ) → กด approve → จบก่อนวันนัด 5 วัน — state machine: submitted → assigned → claimed → (ถาม→ตอบ) → approved → completed / no_show (+ rejected / expired-หัวหน้าโยก / cancelled)
  7. **กันจองเล่น:** ยืนยันอีเมล + 1 คิว active ต่อเลขพาสปอร์ต + เช็คลิสต์บังคับติ๊กครบ + คำถาม 1 ครั้งของ จนท. เป็นด่านสุดท้าย (no-show cooldown เก็บไว้เฟสหลัง)
  8. **การอนุมัติจากผู้บังคับบัญชา: เจ้าของตัดสินใจว่ายังไม่ต้องขอ** — "เริ่มจากเล็กๆ ก่อน ยังไม่ PR พร้อมจริงค่อยขออนุมัติ ถือเป็นระยะทดลองเปิดให้ใช้งานเท่านั้น" — เงื่อนไขขั้นต่ำที่ตกลงกัน: (ก) เก็บข้อมูลน้อยที่สุด (ชื่อ, เลขพาสปอร์ต, อีเมล) (ข) retention ลบอัตโนมัติหลังวันนัดผ่านไป ~60 วัน (แพทเทิร์นเดียวกับลบรูป 31 วัน) + ป้าย "ระยะทดลองให้บริการ" บนเว็บ
- **สถาปัตยกรรม:** Supabase โปรเจกต์เดิม (ใช้ officer/work_group/public_holiday/แพทเทิร์น RPC+PIN เดิม) + ตารางใหม่: `service_type` (ประเภทงาน + เช็คลิสต์ 2 ภาษา เก็บใน DB ให้ชวนชัยแก้ผ่าน UI ได้ ไม่ต้อง deploy ใหม่เมื่อระเบียบเปลี่ยน), `booking` (สถานะ+วันนัด+ข้อมูลผู้จอง+email token), `booking_qa` (คำถาม-ตอบ 1 คู่) — ริชเมนูไลน์แค่แทรกลิงก์ URL ไม่แตะ LINE API
- **Dependency ที่ต้องเสร็จก่อน:** ตั้ง Resend (RESEND_API_KEY — เจ้าของทำเอง งานเดียวกับ Part 3 เฟส C ที่ค้างอยู่)
- **ลำดับการทำเมื่อเริ่มจริง:** (1) ตั้ง Resend (2) เว็บจอง+เช็คลิสต์+ยืนยันอีเมล (3) ฝั่ง จนท. รับ/ถาม/อนุมัติในพื้นที่หลังเช็กอิน + ฝั่งหัวหน้า (ติ๊กรับคิวรายคน/โยกคิว/สถิติ) (4) reminder/no-show/สถิติ — เฟส 2 คือ งานธุรกิจ/ครอบครัวแบบกองกลางกลุ่มงาน + no-show cooldown
- **⚠️ agent ตัวถัดไป: ห้ามเริ่มเขียนโค้ดเองจนกว่าเจ้าของจะสั่งให้เริ่ม** — ดีไซน์นิ่งแล้วแต่เจ้าของยังไม่ได้สั่ง start

**ค้างจาก spec เดิม (ยังไม่ทำ):**
- Grace Card (P1.3) — ดีไซน์ร่างไว้แล้วใน roadmap ยังไม่เขียนโค้ด — ติดเงื่อนไขเดียวกับ badge ทีม OT (P1.1) คือต้องพึ่ง `work_group` แต่ยังมีเจ้าหน้าที่ 4 คนไม่มีกลุ่ม (อัปเดต 20 ส.ค. 2569 — เดิม 11 คน) เจ้าของยังไม่ตัดสินใจว่าจะจัดกลุ่มยังไง (คุยกันแล้วว่ากลุ่ม 1 คนก็ใช้งานได้ปกติ ไม่กระทบ schema เดิม แต่ยังไม่ได้ข้อสรุปสุดท้าย)
...
**✅ (16 ก.ค. 2569) ชื่อการ์ดเปลี่ยนแล้วตามที่เจ้าของสั่ง:** จาก "🤝 จุดบริการกลุ่มงาน OT" เป็น "🤝 แผนกงานการให้บริการ" — แก้ `<h2>` ในทั้ง `dashboard.html`/`report.html` + ข้อความอ้างอิงชื่อการ์ดใน `dashboard.html` (การ์ด "จัดการกลุ่มงาน OT") + code comment ทั้ง 2 ไฟล์ ไม่กระทบ backend/RPC เลย รวม push พร้อม P1b รอบนี้
...
---
## 32. P1b — เหรียญทอง/เงิน เช็กอินคนแรก-คนที่สองของวัน (migration 30, daily_rank) — ✅ เสร็จสมบูรณ์ทั้งหมด รวม push (16 ก.ค. 2569)

**สถานะ: backend (migration 30 รันจริงบน Supabase แล้ว) + frontend (`index.html`/`dashboard.html`/`report.html`) + E2E test ครบทุกเคส verify ผ่านหมด + push ขึ้น git สำเร็จแล้ว (commit `a98f4c7`) + verify บนเว็บจริงผ่าน — ไม่มีงานค้าง**

**ที่มา:** ดีไซน์ยืนยันไว้แล้วตั้งแต่ 13 ก.ค. 2569 (ดูข้อ 7 เดิม) เจ้าของสั่งให้เริ่มทำจริง แบ่งงานเป็น 4 ก้อน: backend, frontend index.html, frontend dashboard/report.html, E2E+docs+push — ทำตามลำดับนี้ (backend ก่อนเพราะ frontend ต้องพึ่ง field ใหม่)

### 32.1 สถาปัตยกรรมที่ใช้จริง (ตรงตามดีไซน์ที่ตกลงไว้ทุกจุด)

- คอลัมน์ใหม่ `check_in.daily_rank smallint check (daily_rank is null or daily_rank in (1,2))`
- ขอบเขต: นับรวมทั้งหน่วยงานเป็นกลุ่มเดียว (ไม่แยกตามกลุ่มงาน OT ของ P1a), นับเฉพาะเช็กอิน "สมบูรณ์" (`incomplete_checkin = false`) เท่านั้นที่มีสิทธิ์ติดอันดับ
- `do_check_in_impl`: ก่อน insert นับ `count(*) from check_in where local_date = v_today and incomplete_checkin = false` — ถ้า count=0 ได้ rank 1, count=1 ได้ rank 2, มากกว่านั้นหรือเช็กอินนี้เอง incomplete ได้ null

- `do_get_today_status_impl`, `do_supervisor_get_today_impl`: คืน `daily_rank` เพิ่มในผลลัพธ์ (ใช้ signature เดิม ไม่ต้อง DROP)
- `do_get_my_month_stats_impl`: เพิ่ม `gold_count`/`silver_count` (นับจาก `daily_rank = 1`/`= 2` ในช่วงเดือนเดียวกับสถิติอื่น)
- Frontend: `index.html` แสดงข้อความยินดี 🥇/🥈 หลังเช็กอินสำเร็จ (`showResult`/`showAlreadyCheckedIn`) + สถิติสะสมรายเดือนในการ์ดสถิติส่วนตัว (`loadMyStats`) — `dashboard.html`/`report.html` แสดงไอคอนเหรียญในตารางเช็กอินวันนี้ (`medalTag`) ตามแพทเทิร์นเดียวกับ `holdTag`/`purpleTag` เดิม

### 32.2 ⚠️ บั๊กที่พบระหว่าง E2E test รอบนี้ — `do_supervisor_get_today_impl` ไม่มี `daily_rank` จริง ทั้งที่สรุปงานรอบก่อนอ้างว่าแก้แล้ว

หลังเรียก `do_supervisor_get_today` จริงผ่าน RPC (ใช้บัญชีทดสอบหัวหน้า PIN "สมาชิกใหม่ที่ขยันขันแข็ง" ตามกติกาข้อ 2.13) พบว่าคำตอบไม่มีคีย์ `daily_rank` เลย ทั้งที่สรุปงานจากก่อน compaction ของ session อ้างว่าแก้ไปแล้ว — วินิจฉัยด้วย `pg_get_functiondef(oid)` เทียบ source จริงบน DB พบว่าฟังก์ชันยังเป็นเวอร์ชันเก่า ไม่มี `c.daily_rank` ในคำสั่ง select เลย (การแก้ที่สรุปไว้ไม่เคยถูกรันจริง น่าจะเกิดจาก Monaco editor corruption/CDP timeout ที่เกิดขึ้นบ่อยเวลาพิมพ์ SQL ยาวๆ ผ่านเบราว์เซอร์ในรอบก่อน) — **แก้ทันทีในรอบนี้** ด้วย `CREATE OR REPLACE FUNCTION` (signature เดิม ไม่ต้อง DROP) ผ่านฟังก์ชันช่วย `tmp_exec_sql` (ส่ง SQL เต็มทาง `fetch()` ตรง แทนพิมพ์ผ่าน Monaco — เร็วกว่าและไม่เจอปัญหา corruption เลย) verify ผ่านทันที **บันทึกเป็นกติกาถาวรแล้วที่ข้อ 2.15**

### 32.3 เทคนิคใหม่ที่ค้นพบรอบนี้: ใช้ `fetch()` + service_role key เรียก RPC ช่วย (`tmp_exec_sql`/`tmp_query_json`) แทนพิมพ์ SQL ผ่าน Monaco editor ได้เกือบทั้งหมด

แทนที่จะพิมพ์ SQL ยาวๆ ลง Monaco editor (เจอปัญหา CDP timeout/corruption บ่อยในรอบก่อนๆ) รอบนี้สร้างฟังก์ชันช่วย 2 ตัวชั่วคราว:
- `tmp_query_json(p_sql text)` — รัน SELECT แล้วคืนผลเป็น JSON (ใช้ query/verify)
- `tmp_exec_sql(p_sql text)` — รัน `EXECUTE p_sql` ตรงๆ ไม่ wrap (ใช้ได้กับ UPDATE/DELETE/INSERT/CREATE OR REPLACE FUNCTION/DROP — ครอบคลุมเกือบทุกอย่างที่ทำผ่าน Monaco ได้)

จากนั้นเรียกทั้งสองผ่าน `fetch()` (ด้วย service_role key จาก `.service_role_key.local.txt`) จาก `javascript_tool` บนแท็บ Supabase Dashboard ที่เปิดอยู่ — **ผลลัพธ์: ทำ E2E test ทั้งหมด (setup, เรียก RPC จริงด้วย anon key, verify, cleanup) ได้เร็วกว่าและเชื่อถือได้กว่าการพิมพ์ผ่าน Monaco editor มาก ไม่เจอปัญหา CDP timeout/corruption แม้แต่ครั้งเดียวตลอดรอบนี้** — **ข้อควรระวัง:** `tmp_exec_sql` ต้องมีอยู่ก่อนจะ DROP ตัวมันเอง (อย่า DROP ทิ้งก่อนใช้ DROP ตัวอื่นที่ยังต้องพึ่งมัน — เจอเองรอบนี้ ต้องพึ่ง Monaco พิมพ์คำสั่งสั้นๆ 1 บรรทัดกู้สถานการณ์) — **แนะนำ agent ตัวถัดไป: ใช้เทคนิคนี้แทนการพิมพ์ Monaco editor ยาวๆ ทุกครั้งที่เป็นไปได้** เหลือแค่กรณีที่ต้อง"สร้าง"ฟังก์ชันตัวแรกที่ยังไม่มี exec helper อยู่เท่านั้นที่จำเป็นต้องพิมพ์ผ่าน Monaco (เช่น bootstrap ฟังก์ชัน `tmp_exec_sql` ตัวแรกในแต่ละเซสชัน)

### 32.4 E2E Test — ทดสอบผ่าน RPC จริงครบทุกเคส (ไม่ใช่แค่อ่านโค้ด)

ใช้ officer ทดสอบ "สมาชิกใหม่" (`bcfe3d7c-ac87-4d6e-b684-a0072c1b9614`) ตั้ง PIN ชั่วคราว + เปิด `checkin_open_after` ชั่วคราวเป็น `00:00` (เวลาทดสอบจริงคือ ~00:40 น. เวลาไทย ก่อน 07:00 ปกติ) ทดสอบผ่าน anon key เหมือน client จริง:

1. **เช็กอินสมบูรณ์ครั้งแรกของวัน** (พิกัดตรงสำนักงาน) → `daily_rank: 1` ✅ ยืนยันซ้ำผ่าน `do_get_today_status` และ query แถวที่เก็บจริงในตาราง ตรงกันหมด
2. **เช็กอินไม่สมบูรณ์** (ไม่ส่งพิกัด) → `daily_rank: null`, `incomplete: true` ✅
3. **เช็กอินสมบูรณ์เมื่อมีคนสมบูรณ์แล้ว 1 คน** (สร้าง officer ปลอม + แถวเช็กอินสมบูรณ์ปลอม 1 แถว) → `daily_rank: 2` ✅ (ยืนยันเพิ่มผ่าน `do_supervisor_get_today` ด้วยบัญชีทดสอบหัวหน้า PIN — พบบั๊กข้อ 32.2 ตรงจุดนี้เอง)
4. **เช็กอินสมบูรณ์เมื่อมีคนสมบูรณ์แล้ว 2 คน** (officer ปลอม 2 คน) → `daily_rank: null` (เกินโควตาอันดับ) ✅
5. **Edge case: เช็กอินไม่สมบูรณ์ไม่กินสิทธิ์อันดับ** — สร้างแถวปลอม incomplete 1 แถว แล้วเช็กอินสมบูรณ์จริง → ยังได้ `daily_rank: 1` (ไม่ใช่ 2) ✅ ยืนยันว่า `v_today_complete_count` นับเฉพาะ `incomplete_checkin = false` ถูกต้อง
6. **`do_get_my_month_stats`** — ตรวจระหว่างเคส 1 ยังไม่ลบ พบ `gold_count: 1, silver_count: 0` ตรงกับที่คาดไว้ ✅

**Cleanup:** ลบแถว check_in ทดสอบทั้งหมด + officer ปลอม 2 ตัว, คืนค่า `settings.checkin_open_after` กลับ `07:00`, รีเซ็ต `pin_hash`/`pin_fail_count`/`pin_locked_until` ของทั้ง officer ทดสอบและบัญชีทดสอบหัวหน้า PIN กลับ `null`/`0`/`null`, DROP `tmp_exec_sql`/`tmp_query_json` ทิ้งครบ — verify ด้วย query สุดท้ายว่า `leftover_checkins=0, leftover_fake_officers=0` ✅

### 32.5 Frontend — verify syntax แล้ว push แล้ว (ดูข้อ 32.6)

`index.html`: เพิ่ม CSS `.medal-box`, div `#resultMedalBox`/`#alreadyMedalBox`, แถวสถิติทอง/เงินในการ์ดสถิติส่วนตัว, ฟังก์ชัน `showMedalBadge()`, ต่อยอด `loadMyStats()`/`showResult()`/`showAlreadyCheckedIn()`/`resetForm()` — verify ผ่าน `node --check` (extract เฉพาะ script body)

`dashboard.html`/`report.html`: แก้ `renderCheckinTable()` เพิ่มตัวแปร `medalTag` (แพทเทิร์นเดียวกับ `holdTag`/`purpleTag` เดิม) ต่อเข้า HTML ของแถวตาราง — `dashboard.html` ต้องเพิ่ม `daily_rank` ใน `.select()` query ด้วย (ข้อมูลมาจาก Supabase client โดยตรงไม่ผ่าน RPC), `report.html` ไม่ต้องแก้ query เพราะข้อมูลมาจาก RPC `do_supervisor_get_today` ที่คืน `daily_rank` อยู่แล้ว (หลังแก้บั๊กข้อ 32.2)

### 32.6 Push + verify บนเว็บจริง — ✅ สำเร็จ (16 ก.ค. 2569)

เจ้าของตรวจไฟล์ผ่าน `present_files` แล้วสั่ง "push เลย" — sync ไฟล์ที่แก้ทั้งหมด (`index.html`, `dashboard.html`, `report.html`, `CLAUDE.md`, ไฟล์ใหม่ `migrations/30_daily_rank_medals.sql`) เข้า git working copy ที่ `/tmp/rg30` (clone สดจาก GitHub) แล้ว `node --check` ผ่านทั้ง 3 ไฟล์ HTML ก่อน commit — เจ้าของส่ง GitHub fine-grained PAT มาเองตามขั้นตอนเดิม (ข้อ 23.4) commit `a98f4c7` push สำเร็จ

**Verify บนเว็บจริง:** `fetch()` เช็ก marker string บน `rayongimm.link` ทั้ง 3 ไฟล์ — `index.html` มี `MedalBox`, `dashboard.html`/`report.html` มี `medalTag` + ชื่อการ์ดใหม่ "แผนกงานการให้บริการ" ครบ ไม่เหลือชื่อเก่า "จุดบริการกลุ่มงาน OT" เลยสักที่

---

## 33. P1.4 — เช็คเอาต์ตอนเย็น ไม่บังคับ (migration 31, check_out) — ✅ เสร็จสมบูรณ์ทั้งหมด รวม push (16 ก.ค. 2569)

**สถานะ: backend (migration 31 รันจริงบน Supabase แล้ว) + frontend (`index.html`/`dashboard.html`/`report.html`) + E2E test ครบทุกเคส verify ผ่านหมด + push ขึ้น git สำเร็จแล้ว — ไม่มีงานค้าง**

**ที่มา:** เจ้าของขอปรึกษาแผนงาน P1 ที่เหลือ (เช็คเอาต์ตอนเย็น + Grace Card) ในสไตล์ "คู่คิดเชิงกลยุทธ์" — หารือกันแล้วตัดสินใจ 3 จุดสำคัญก่อนเริ่มเขียนโค้ด (ดูข้อ 33.1) แล้วเริ่มเช็คเอาต์เย็นก่อน Grace Card เพราะไม่ต้องพึ่ง `work_group` เลย (Grace Card ต้องพึ่งกลุ่มงานที่ยังจัดไม่ครบ 11/20 คน)

### 33.1 การตัดสินใจที่เบี่ยงจากดีไซน์ร่างเดิมใน `90_ROADMAP_v2_PLAN.md` (ยืนยันกับเจ้าของแล้วก่อนเขียนโค้ด)

1. **ล็อกเช็คเอาต์ได้ครั้งเดียวเด็ดขาดต่อวัน** — ดีไซน์ร่างเดิมเสนอ "กดได้หลายครั้ง ครั้งล่าสุดชนะ" (upsert) เผื่อเคส "เช็คเอาต์ 18:00 แล้วโดนเรียกให้อยู่ต่อถึง 20:00" แต่เจ้าของยืนยันว่าไม่จำเป็น อยากให้เช็คเอาต์ได้ครั้งเดียว (ไม่บังคับว่าต้องทำ แต่ทำแล้วแก้ไม่ได้) — trade-off ที่ยอมรับ: คนที่เช็คเอาต์เร็วแล้วอยู่ต่อจริงจะไม่ได้เครดิตระดับ "อยู่ดึก" ย้อนหลัง (ยอมรับความเสี่ยงนี้แล้ว เพราะออกแบบเรียบง่ายกว่า ไม่ต้องมี UI แก้ไข/ยกเลิกแยก)
2. **เกณฑ์เวลา tier (18:00 อยู่เย็น / 20:00 อยู่ดึก) เขียนตายตัวในตาราง ไม่มี UI แก้ไขในรอบนี้** — ดีไซน์ร่างเดิมเสนอ RPC `do_get/set_checkout_tiers` ให้ชวนชัยแก้ได้ผ่าน UI แต่เจ้าของเลือกเขียนตายตัวไปก่อน (เก็บ `checkout_recognition_tier` เป็นตารางแยกไว้แล้ว ถ้าต้องการ UI แก้ทีหลังทำเพิ่มได้โดยไม่ต้อง migration ใหม่)
3. **หน้าเช็คเอาต์ใช้ URL เดิม (`index.html`)** — เจ้าของถามว่าเจ้าหน้าที่จะกดเข้าตรงไหน ตอบว่าไม่ต้องทำหน้าใหม่ ใช้ปุ่มรองในหน้า login เดิม (ตอบคำถามนี้ก่อนเริ่มเขียนโค้ดจริง)

### 33.2 สถาปัตยกรรม Backend ที่ใช้จริง

ตาราง `check_out` (unique `(officer_id, local_date)` — บังคับล็อกครั้งเดียวที่ระดับ DB ไม่ใช่แค่ frontend), `checkout_recognition_tier` (seed 2 แถว: `1/'18:00'/อยู่เย็น/🌙`, `2/'20:00'/อยู่ดึก/🌙🌙`) — RPC `do_check_out`/`do_get_my_checkout_today` สร้างตามแพทเทิร์น thin-wrapper เดียวกับ PIN rate limiting เดิม (migration 25: `check_and_count_pin` ก่อนเสมอ, `_impl` แยก revoke จาก anon/authenticated)

**Hard block ตามกติกาข้อ 3:** `do_check_out_impl` เช็กว่ามีแถว `check_in` ของวันนี้ก่อนเสมอ ไม่มีคืน error `not_checked_in_yet` — **GPS ไม่บล็อกเลย** (ต่างจากเช็กอินเช้าที่มีสถานะม่วง) เก็บ `distance_m` ไว้เงียบๆ ในทุกกรณี ทดสอบจริงด้วยพิกัดกรุงเทพฯ (ห่างจากสำนักงานจริง ~134 กม.) ยืนยันว่าเช็คเอาต์ผ่านปกติ (`ok:true`) ไม่มีการเตือน/บล็อกใดๆ

**แก้ RPC เดิม 4 ตัว (LEFT JOIN `check_out`, signature เดิมทุกตัว ไม่ต้อง DROP):**
- `do_get_my_month_stats_impl` — เพิ่ม `checkout_tiers` (array นับจำนวนครั้งต่อ tier) + `checkout_total`
- `do_supervisor_get_today_impl` — เพิ่ม `checked_out_at`/`tier_label`/`tier_emoji`
- `do_supervisor_get_history_impl` — เพิ่มฟิลด์เดียวกัน (⚠️ ฟังก์ชันนี้ไม่มี `daily_rank` มาตั้งแต่ P1b — ช่องว่างเดิม ไม่ได้แก้ในรอบนี้เพราะนอกขอบเขต P1.4)
- `do_get_history` — เพิ่มฟิลด์เดียวกัน **+ เพิ่ม `daily_rank` ไปด้วยพร้อมกัน** (ดูข้อ 33.3 ทำไมถึงแก้เพิ่ม)

### 33.3 ⚠️ พบช่องว่างเดิมจาก P1b ระหว่างทำ: `do_get_history` (auth-only, ใช้โดย `dashboard.html`) ไม่เคยมี `daily_rank`

ตอนเตรียมแก้ `do_get_history` ให้มี checkout fields พบว่าฟังก์ชันนี้ (คนละตัวกับ `do_supervisor_get_history_impl` ที่เป็น PIN wrapper — `do_get_history` คือเวอร์ชัน `auth.uid()`-gated ที่ `dashboard.html` เรียกตรงสำหรับตารางประวัติ) ไม่มี `daily_rank` เลยตั้งแต่ P1b เพิ่มฟีเจอร์เหรียญทอง/เงิน (P1b แก้แค่ `do_supervisor_get_today_impl`/`do_get_today_status_impl`/`do_get_my_month_stats_impl`/`do_check_in_impl` แต่ลืม `do_get_history`) — ผลคือ **เหรียญทอง/เงินไม่เคยแสดงในตารางประวัติย้อนหลังของ `dashboard.html` เลยตั้งแต่ 16 ก.ค. เช้า (P1b เสร็จ) จนถึงตอนพบบั๊กนี้ (16 ก.ค. เย็น)** ยืนยันจาก `pg_get_functiondef` ว่า source เดิมไม่มี `c.daily_rank` ในคำสั่ง select — แก้พร้อมกันในรอบนี้เพราะกำลังแก้ไฟล์เดียวกันอยู่แล้ว (เพิ่ม LEFT JOIN check_out + `c.daily_rank`) ไม่ต้อง DROP (signature เดิม) verify `count(*)=1` ผ่าน

**บทเรียน:** ฟีเจอร์ที่มี "ฟังก์ชันคู่แฝด" (auth-only กับ PIN-wrapper สำหรับ 2 หน้าต่างกัน) เสี่ยงแก้ไม่ครบทั้งคู่ได้ง่าย — เวลาจะเพิ่มฟิลด์ใหม่ในฟังก์ชันตระกูล `do_get_history`/`do_supervisor_get_history*`/`do_get_today_status*`/`do_supervisor_get_today*` ควรเช็กครบทั้ง 2 เวอร์ชันเสมอ ไม่ใช่แค่ตัวที่นึกถึงก่อน

### 33.4 Frontend

**`index.html`** — เพิ่มปุ่มรอง "🌙 เช็คเอาต์ตอนเย็น (ไม่บังคับ)" ในหน้า login (ใต้ปุ่ม "เปิดกล้องเพื่อเช็กอิน" เดิม ใช้ officer/PIN ที่กรอกไว้ร่วมกัน) — คลิกแล้ว precheck 2 ชั้นก่อนเปิดกล้องเสมอ (กันเสียเวลาถ่ายรูปฟรี + กันรูปกำพร้าใน Storage ตามแพทเทิร์นเดียวกับข้อ 24): (1) เรียก `do_get_today_status` เช็กว่าเช็กอินเช้าแล้วหรือยัง ถ้ายัง → บล็อกทันทีด้วยข้อความมิตรภาพ ไม่เปิดกล้องเลย (2) เรียก `do_get_my_checkout_today` เช็กว่าเช็คเอาต์ไปแล้วหรือยัง ถ้าแล้ว → โชว์หน้า "เช็คเอาต์ไปแล้ววันนี้" พร้อม tier ที่ได้ทันที ไม่เปิดกล้องซ้ำ — ถ้าผ่านทั้งคู่ค่อยเปิดกล้อง (ใช้ `screen-camera` เดิมร่วมกับเช็กอินเช้า แยกด้วย flag `checkoutMode` เปลี่ยนแค่ข้อความหัวข้อ) ไปหน้ายืนยันใหม่ (`screen-checkout-confirm` — หมายเหตุไม่บังคับ ไม่มีขั้นต่ำตัวอักษร ต่างจากเช็กอินเช้า) แล้วอัปโหลดรูป (bucket เดิม `checkin-photos` เติม prefix `checkout_`) + เรียก `do_check_out` — ผลลัพธ์แสดงป้าย tier (ถ้าถึงเกณฑ์) หรือข้อความกลางๆ (ถ้ายังไม่ถึง) ที่หน้า `screen-checkout-result`

**`dashboard.html`/`report.html`** — เพิ่มคอลัมน์ "🌙 เช็คเอาต์" ในตาราง `renderCheckinTable` ที่ใช้ร่วมกันทั้งวันนี้/ประวัติ (แสดงเวลา + ป้าย tier ถ้ามี, "-" ถ้ายังไม่เช็คเอาต์) — `report.html` ไม่ต้องแก้ query เพราะข้อมูลมาจาก RPC `do_supervisor_get_today`/`do_supervisor_get_history` ที่คืนฟิลด์ checkout อยู่แล้ว (หลังแก้ backend ข้างต้น) — `dashboard.html` ตาราง "วันนี้" ดึงข้อมูลตรงจาก `sb.from("check_in")` ไม่ผ่าน RPC จึงต้อง query `check_out` แยกอีกคิวรีสำหรับวันนี้แล้ว merge เข้ากับแถว check_in เองฝั่ง client ด้วย `officer_id` (เพิ่ม `officer_id` เข้า select list ของ check_in ด้วย) — ตาราง "ประวัติ" ของ `dashboard.html` ใช้ RPC `do_get_history` (auth-only) ที่แก้ไปแล้วในข้อ 33.2/33.3 จึงได้ข้อมูลมาตรงๆ ไม่ต้อง merge — อัปเดต CSS mobile card mapping (`nth-child`) ทั้ง 2 ไฟล์ให้ตรงกับคอลัมน์ใหม่ที่เพิ่มเข้าไป

### 33.5 E2E Test — ทดสอบผ่าน RPC จริงครบทุกเคส

ใช้ officer ทดสอบ "สมาชิกใหม่" (`bcfe3d7c-ac87-4d6e-b684-a0072c1b9614`) + บัญชีทดสอบหัวหน้า PIN "สมาชิกใหม่ที่ขยันขันแข็ง" (`20f2b74f-e15b-4b77-94a7-eb8e8adb1650`) ตามกติกาข้อ 2.13:

1. **เช็คเอาต์ก่อนเช็กอิน** → `{ok:false, error:"not_checked_in_yet"}` ✅
2. **เช็คเอาต์หลังเช็กอินสำเร็จ (ก่อนเวลา tier ใดๆ, ~07:12 น.)** → `ok:true, tier_order:null` ✅
3. **เช็คเอาต์ซ้ำทันที (ล็อกครั้งเดียว)** → `{ok:false, error:"already_checked_out"}` ✅
4. **ทดสอบ tier assignment** — ลบแถวทดสอบ + ปรับ `checkout_recognition_tier.threshold_time` ชั่วคราวให้ต่ำกว่าเวลาปัจจุบัน แล้วเช็คเอาต์ใหม่ → tier 2 (`อยู่ดึก 🌙🌙`) ✅ แล้วทดสอบ tier 1 เดี่ยว (`อยู่เย็น 🌙`) ✅ — คืนค่า threshold กลับ `18:00`/`20:00` เดิม
5. **`do_get_my_checkout_today`** คืนสถานะที่บันทึกไว้ถูกต้อง (เวลา + tier) ✅
6. **`do_get_my_month_stats`** คืน `checkout_tiers`/`checkout_total` ถูกต้องตามที่เช็คเอาต์ไป ✅
7. **GPS ไกล 134 กม. (พิกัดกรุงเทพฯ) ไม่บล็อกการเช็คเอาต์** → `ok:true, distance_m:134023` บันทึกเงียบๆ ไม่มี error/คำเตือน ✅
8. **ยืนยัน LEFT JOIN check_out ของ `do_get_history`/`do_supervisor_get_history` ถูกต้อง** — insert check_in+check_out ทดสอบแล้ว query ทั้ง raw SQL join ตรงๆ และเรียก `do_supervisor_get_history` จริงผ่าน PIN ยืนยัน `checked_out_at`/`tier_label`/`tier_emoji` ตรงกันหมด ✅ (verify `do_get_history` ทางอ้อมผ่าน raw join เพราะไม่มีบัญชีทดสอบที่มี Supabase Auth session จริงให้เรียกตรง — ยืนยัน auth gate ยังทำงานถูกต้องแยกต่างหากด้วยการเรียกด้วย anon key แล้วได้ `not_supervisor` ตามที่ควรเป็น)

**Cleanup:** ลบแถว check_in/check_out ทดสอบทั้งหมด, คืนค่า `checkout_recognition_tier` กลับ `18:00`/`20:00`, รีเซ็ต `pin_hash`/`pin_fail_count`/`pin_locked_until` ของทั้ง 2 บัญชีทดสอบกลับ `null`/`0`/`null`, DROP `tmp_exec_sql`/`tmp_query_json` ทิ้งครบ (ยืนยันด้วยการเรียก RPC ซ้ำแล้วได้ `404 PGRST202` ทั้งคู่) — verify ด้วย query สุดท้ายว่า `leftover_checkin=0, leftover_checkout=0` ✅

### 33.6 Verify syntax — วิธีใหม่ที่ใช้รอบนี้ (bash mount staleness รุนแรงขึ้นอีก)

พบว่า bash `cat`/`python open()` อ่านไฟล์ `index.html`/`dashboard.html`/`report.html` ในโฟลเดอร์โปรเจกต์ได้เนื้อหาที่ตัดจบกลางประโยค (ไม่ตรงกับที่ Edit tool เพิ่งแก้ไปจริง) แม้แต่ `wc -c`/`stat` ก็เห็น mtime ค้างหลายชั่วโมงถึงข้ามวัน — ตรงกับกติกาข้อ 2.10 ที่บันทึกไว้แล้ว **ยืนยันอีกครั้งว่า `Grep`/`Read` tool เท่านั้นที่เชื่อถือได้** — วิธี verify syntax ที่ใช้จริงรอบนี้: อ่านไฟล์เต็มผ่าน `Read` tool (หลายหน้าถ้าไฟล์ยาว) แล้ว `Write` ไปที่ outputs (ไม่ติดบั๊ก) ก่อนรัน `node --check` (ทำแบบนี้กับ `index.html` เต็มไฟล์) — ส่วน `dashboard.html`/`report.html` (ไฟล์ใหญ่กว่ามาก ~1700 บรรทัด) ใช้วิธีที่ประหยัดกว่า: สกัดเฉพาะฟังก์ชันที่แก้จริง (`loadTodayCheckins`/`renderTodayTable`/`renderCheckinTable`) มา wrap เป็นไฟล์ทดสอบเดี่ยวพร้อม stub ของ `document`/`sb` แล้ว `node --check` เฉพาะส่วนนั้น — ครอบคลุมความเสี่ยง syntax error ในจุดที่แก้จริงโดยไม่ต้อง reconstruct ทั้งไฟล์ (ซึ่งมีความเสี่ยง transcription error จากการพิมพ์ยาวเองสูงกว่า ไม่คุ้มกับ risk ที่ลดได้)

### 33.7 Push — ✅ สำเร็จ (16 ก.ค. 2569)

เจ้าของตรวจไฟล์ผ่าน `present_files` แล้วสั่ง "push เลย" — sync ไฟล์ที่แก้ทั้งหมด (`index.html`, `dashboard.html`, `report.html`, `CLAUDE.md`, ไฟล์ใหม่ `migrations/31_evening_checkout.sql`) เข้า git working copy ที่ `/tmp/rg31` (clone สดจาก GitHub) แล้ว `node --check` ผ่านทั้ง 3 ไฟล์ HTML ก่อน commit — เจ้าของส่ง GitHub fine-grained PAT มาเองตามขั้นตอนเดิม (ข้อ 23.4) — **⚠️ เจอบั๊ก bash-mount staleness รุนแรงที่สุดเท่าที่เคยเจอรอบนี้: `cp` ไฟล์ `index.html`/`dashboard.html`/`report.html`/`CLAUDE.md` จากโฟลเดอร์โปรเจกต์ตรงๆ ได้เนื้อหาที่ตัดจบกลางไฟล์ (ไม่มี `</html>` เลยสักไฟล์) แก้โดยใช้เนื้อหาที่ยืนยันแล้วผ่าน `Read` tool (ground truth) เป็นฐานแทนการ `cp` ตรงจาก mount ทั้งหมด** ดูรายละเอียดกติกาที่ปรับปรุงในข้อ 2.10

---

---

## 34. ลบปุ่มลัดหมายเหตุ "⚡ ปฏิบัติงานปกติ (กรอกให้อัตโนมัติ)" ออกจาก `index.html` + บันทึกกติกาใหม่เรื่องบัญชีทดสอบ "สมาชิกใหม่" — ✅ เสร็จสมบูรณ์ push แล้ว (19 ก.ค. 2569)

**สถานะ: แก้ไฟล์ + verify syntax ผ่าน + push ขึ้น git สำเร็จแล้ว**

**ที่มา 1 (ฟีเจอร์):** เจ้าของส่งภาพหน้าจอ "ตรวจสอบก่อนยืนยัน" มาพร้อมคำสั่งให้ลบปุ่มลัด "⚡ ปฏิบัติงานปกติ (กรอกให้อัตโนมัติ)" ที่อยู่ใต้ช่องหมายเหตุออก — เหตุผล: "ต้องการให้เจ้าหน้าที่รายงานสั้นสั้นทุกๆวัน ตามสถานการณ์จริง ไม่อยากให้มีกรอกอัตโนมัติ" (ปุ่มนี้เพิ่มไว้ตั้งแต่ UX quick win รอบ 2 เมื่อ 10 ก.ค. 2569 ดูข้อ 26.2 — ตอนนี้เจ้าของประเมินแล้วว่าขัดกับเจตนาที่อยากให้รายงานสถานการณ์จริงรายวัน จึงสั่งถอดออก) เจ้าของยังถามด้วยว่าช่องหมายเหตุ "จดจำคำเมื่อวาน" หรือไม่ — ตรวจสอบโค้ดแล้วยืนยันว่าช่องหมายเหตุ **ไม่เคยมี localStorage persistence เลย** (`noteInput` ถูกรีเซ็ตเป็น `""` ทุกครั้งใน `resetConfirmFields()`/`resetForm()`) สิ่งที่เจ้าของเห็นคือปุ่มลัดนี้เองที่กรอกข้อความเดิมซ้ำให้ทุกครั้งที่กด ไม่ใช่การจดจำค่าเก่าจริง — พอลบปุ่มออกแล้วปัญหานี้หมดไปเองในตัว

**แก้ 2 จุดใน `index.html`:** (1) ลบ `<button type="button" class="btn-secondary" id="btnQuickNoteNormal">⚡ ปฏิบัติงานปกติ (กรอกให้อัตโนมัติ)</button>` ออกจาก HTML (2) ลบ JS `addEventListener` handler ของปุ่มนี้ออกจาก `<script>` — ยืนยันด้วย `Grep` ว่าไม่เหลือคำว่า "QuickNote"/"ปฏิบัติงานปกติ" ที่ไหนในไฟล์เลย, อ่านทั้งไฟล์ผ่าน `Read` tool ครบทุกบรรทัดเพื่อยืนยันโครงสร้างไม่พัง แล้ว `node --check` ผ่าน

**ที่มา 2 (กระบวนการทดสอบ):** ระหว่างคุยกัน เจ้าของถามว่าทำไมบัญชีทดสอบ "สมาชิกใหม่" ขึ้น "มาเลท 1 ครั้ง ขาดเช็คอิน 8 ครั้ง" ทั้งที่รวมเช็กอินเดือนนี้แค่ 1 ครั้ง และสงสัยว่าข้อมูลหายไหม — ตรวจสอบจริงผ่าน REST (query ตาราง `officer`/`check_in` ด้วย service_role key) พบว่าไม่ใช่บั๊ก แต่เป็นผลจากกติกาเดิมที่ agent ลบแถวทดสอบ + รีเซ็ต PIN ทุกรอบหลังทดสอบเสร็จ ทำให้บัญชีนี้ไม่มีประวัติสะสมจริงเลย (ยืนยันคู่ขนานว่าเจ้าหน้าที่จริงทั้ง 19 คนมีจำนวนเช็กอินเดือนนี้ปกติ 9-11 ครั้งต่อคน ไม่มีข้อมูลหาย) — เจ้าของยืนยันว่าไม่ใช่บั๊กแต่ไม่ต้องการพฤติกรรมนี้อีกต่อไป จึงสั่งเปลี่ยนกติกา (บันทึกไว้แล้วในหัวข้อ 12 ก.ค. 2569 ด้านบน — เพิ่มย่อหน้า "16 ก.ค. 2569 เย็น" ต่อท้าย)

**Push:** เจ้าของยืนยัน "push เลย" ด้วย GitHub PAT เดิมที่ยังไม่หมดอายุ (ยืนยันตั้งใจใช้ต่อ ไม่ได้พลาดคัดลอกซ้ำ) — sync `index.html` (verify แล้วผ่าน `node --check`) เข้า git working copy ที่ `/tmp/rg34` (clone สดจาก GitHub, HEAD เดิมคือ `1fc7ffc`) พร้อม `CLAUDE.md` ฉบับล่าสุด (เพิ่มย่อหน้ากติกาใหม่เรื่องบัญชี "สมาชิกใหม่" ในข้อ 12 ก.ค. + เพิ่มหัวข้อนี้) แล้ว commit+push ขึ้น `main`

---

## 35. กู้แบนเนอร์น้องริมมี่ (Rimmi for Officer) ที่หายจาก manual Netlify Drop — ✅ push แล้ว commit `1021481` (22 ก.ค. 2569)

**สถานะ: เสร็จสมบูรณ์ — กู้โค้ด+รูปต้นฉบับกลับ push ขึ้น git (รูป `rimmi-approved-mockup.png` เข้า repo แล้ว) verify บนเว็บจริงผ่าน**

**ที่มา:** เจ้าของแจ้งว่าแบนเนอร์น้องริมมี่ด้านล่างหน้าเช็กอิน (ที่ลิงก์ไป `https://rimmiforofficer.netlify.app/#`) หายไปหลัง push งานล่าสุด

**สาเหตุ (บทเรียนสำคัญ — กติกาใหม่ข้อ 2.16):** แบนเนอร์ถูก **เอเจนต์อีกตัว (เซสชันโปรเจกต์ริมมี่) deploy แบบ manual ผ่าน Netlify Drop โดยตรง ไม่ได้ commit เข้า git เลย** (ไล่ดู git history ครบ 23 commit = ไม่มีแบนเนอร์สักที่) — Netlify ตั้ง auto-publish จาก git ไว้ พอ agent นี้ push งานผ่าน git (commit ก่อนหน้า) Netlify build จาก git ทับ manual deploy → แบนเนอร์หาย

**วิธีกู้:** ไปที่ Netlify deploy history เจอ manual deploy เดิม ("Rayong check-in visual refresh and Rimmi banner", deploy id `6a5c52a9...`) → navigate ไปที่ deploy permalink `https://<deployid>--comfy-gaufre-b6b83e.netlify.app/index.html` ดึง HTML ต้นฉบับ → สกัดโค้ดแบนเนอร์เป๊ะ (`<a class="rimmi-promo">` + `<img class="promo-approved-art" src="rimmi-approved-mockup.png">` + CSS 14 กฎ ที่มี `.rimmi-promo` aspect-ratio 775/385 + crop รูปด้วย `top:-328.052%` ฯลฯ) → เจ้าของคัดลอกรูป `rimmi-approved-mockup.png` (2MB) เข้าโฟลเดอร์โปรเจกต์ให้ → graft ลงไฟล์ git ปัจจุบัน (คงการลบ QuickNote ไว้) วางแบนเนอร์ไว้ท้าย `#screen-login` ก่อน `</section>` (โชว์เฉพาะหน้า login) → commit ทั้ง `index.html` + `rimmi-approved-mockup.png` เข้า git

**⚠️ กติกาใหม่ข้อ 2.16 (กันซ้ำ):** โปรเจกต์นี้ deploy ผ่าน **git + Netlify auto-publish จาก main เท่านั้น** — **ห้ามใครก็ตาม (รวมเอเจนต์เซสชันอื่น) deploy แบบ manual Netlify Drop เด็ดขาด** เพราะจะโดน git push รอบถัดไปทับหายทันที ถ้ามีหลายเซสชัน/เอเจนต์ทำงานกับเว็บนี้ ทุกการเปลี่ยนแปลงต้องเข้า git เท่านั้น — ถ้าเจอฟีเจอร์บน live ที่ไม่มีใน git ให้สงสัยว่ามาจาก manual deploy แล้วกู้เข้ามาใน git ก่อน push อะไรทับ

---

## 36. เฟส C — แท็บ "เสียงประชาชน" (ติ-ชม) + กล่องคำชมจากประชาชน — ✅ deploy production แล้ว commit `6cae913` (22 ก.ค. 2569)

**สถานะ: เสร็จสมบูรณ์ — apply 3 patch (index/dashboard/report) แบบ additive, backend RPC มีครบแล้ว, push ผ่าน git, verify production ผ่าน**

**ที่มา:** deploy ตาม `phase-c/PHASE_C_DEPLOY_HANDOFF.md` (เขียนโดยเซสชันโปรเจกต์ริมมี่) — เปิดให้หัวหน้าอ่าน feedback (ติ-ชม) จากประชาชน เพื่อปลดล็อกให้ริมมี่เริ่มโยนคนไปเว็บฟีดแบก · ไฟล์ patch + handoff อยู่ที่ `phase-c/` ในโฟลเดอร์โปรเจกต์

**สิ่งที่เพิ่ม (additive ล้วน ลบ 0 บรรทัด):**
- `index.html` (+102) — กล่อง "🌟 คำชมจากประชาชน" โผล่หลังเช็กอิน 2 จอ (result/already) · JS `loadMyPraise()` เรียก RPC `get_my_praise` (anon) · โชว์เฉพาะคำชมที่หัวหน้าอนุมัติแล้ว · `has_discuss` เป็นแค่ boolean "หัวหน้าอยากคุยด้วย" ไม่มีเนื้อหาคำติ
- `dashboard.html` (+264) — แท็บ "💬 เสียงประชาชน" (หัวหน้า auth) · RPC `list_feedback`/`approve_praise`/`review_feedback`/`set_discuss_flag` · **คำติไม่แสดงชื่อเจ้าหน้าที่** · badge ติใหม่ · คำชมถึงเจ้าหน้าที่เมื่อกด "อนุมัติ"
- `report.html` (+264) — แท็บเดียวกันฝั่งหัวหน้า PIN · RPC `do_supervisor_list_feedback`/`do_supervisor_approve_praise`/`do_supervisor_review_feedback`/`do_supervisor_set_discuss_flag` + `list_feedback_counters` (anon)

**ตรวจสอบก่อน deploy (ครบตาม handoff):** (1) verify backend RPC ทั้ง 10 ตัวมีอยู่จริงบน live DB ผ่าน REST + service_role — EXISTS ครบ ไม่ต้องรัน SQL เพิ่ม (2) สำรอง live: git commit `1021481` = live เป๊ะ (เทียบ raw GitHub == live ทุกไบต์) + copy 3 ไฟล์ไป `phase-c/backup-live-20260722-0816/` (3) เช็ก dependency symbols (THAI_MONTHS_SHORT/allWorkGroups/escapeHtml/session) ครบ (4) apply patch `--fuzz=3` สำเร็จ ลบ 0 บรรทัด (5) `node --check` ผ่านทั้ง 3 + ไม่มีฟังก์ชัน/id ซ้ำ + smoke test (stub DOM) รัน init ได้ครบ = หน้าเช็กอินไม่พัง

**⚠️ เบี่ยงจาก handoff 1 จุด (สำคัญ):** handoff ขั้นตอน 5 เขียนให้ deploy ผ่าน **Netlify Drop** แต่โปรเจกต์นี้เปลี่ยนเป็น **git + auto-publish** แล้ว (handoff เขียนจากเซสชันอื่นที่ไม่รู้) — **deploy ผ่าน git push แทน** (ถ้า Drop จะโดน git ทับ + แบนเนอร์หายซ้ำ ดูข้อ 35/กติกา 2.16)

**Verify production:** `fetch()` เช็ก marker บน `rayongimm.link` — index มี PraiseBox + ปุ่มเช็กอิน + แบนเนอร์ครบ, dashboard/report มี group-feedback + list_feedback RPC · โหลดหน้าเช็กอินจริง dropdown ขึ้นครบ 21 รายชื่อ ไม่มี JS error ทำหน้าพัง

**เหลือให้เจ้าของทำเอง:** (1) ทดสอบขั้นสุดท้าย — ล็อกอิน dashboard/report เข้าแท็บเสียงประชาชน กดอนุมัติคำชม/รับเรื่องคำติ (agent ทำแทนไม่ได้ ไม่มีรหัสหัวหน้า) (2) **Part 3 อีเมลแจ้งคำติ** `phase-c/feedback-notify/` (Deno+Resend) ยัง deploy ไม่ได้ — ต้องสมัคร Resend + กรอก `RESEND_API_KEY` เอง (ผู้ช่วย AI กรอก API key ไม่ได้) จนกว่าตั้งเสร็จหัวหน้าต้องเปิด dashboard เช็ก feedback เอง (3) กลับโปรเจกต์ริมมี่ เปลี่ยนสถานะ 11 แถวใน `Rimmi_QA_Database` เป็น "พร้อมใช้" ให้ริมมี่เริ่มโยนคนไปเว็บฟีดแบก

---

## 37. บั๊ก production: ค้นประวัติช่วงยาวใน `report.html` แล้วเว็บค้าง ("กดแล้วไม่ขึ้นอะไร") — ✅ แก้แล้ว push commit `11ff738` (22 ก.ค. 2569)

**สถานะ: วินิจฉัย + แก้ + deploy + วัดผลเทียบก่อน-หลังบน production เสร็จครบ ไม่มีงานค้าง**

**ที่มา:** เจ้าของให้หัวหน้าอีกท่านเข้า `report.html` (ล็อกอิน PIN) ดึงสถิติเช็กอินย้อนหลังทั้งเดือน (6–31 ก.ค.) บนมือถือ แล้ว "กดแล้วไม่ขึ้นอะไร" — ส่วนเจ้าของเองใช้ `dashboard.html` ได้ปกติ

**วินิจฉัย (ทดสอบจริงด้วยบัญชีทดสอบหัวหน้า PIN ตามกติกาข้อ 2.13):** ไม่ใช่บั๊ก backend และไม่ใช่ปัญหาของมือถือ — RPC `do_supervisor_get_history` คืนข้อมูลครบ 303 แถวปกติ, ตารางวาดเสร็จใน 6.7 วิ **แต่หลังจากนั้นแท็บค้างจนไม่ตอบสนอง** (เครื่องมือทดสอบ CDP timeout เอง) สาเหตุคือ `attachPhotoLoaders()` ยิงขอ signed URL ของรูป **ทุกแถวพร้อมกันทันที** = 303 คำขอไปที่ Edge Function `supervisor-photo-url` ซึ่งแต่ละคำขอต้องตรวจ PIN ด้วย bcrypt ฝั่งเซิร์ฟเวอร์ → คิวยาวมาก **วัดจริงได้ค่ามัธยฐาน 21.5 วิ/คำขอ ช้าสุด 39.7 วิ**

**⚠️ ทำไม `dashboard.html` ไม่เจอปัญหานี้ (จุดที่ทำให้เข้าใจผิดว่า "คนใช้ผิด"):** dashboard ล็อกอินด้วย Supabase Auth จริง จึงเรียก `sb.storage.createSignedUrl()` เองฝั่ง client ได้ ไม่ต้องผ่าน Edge Function ทีละรูป — ส่วน report.html เป็นหัวหน้า PIN ไม่มี auth session เลยต้องยิง Edge Function ทุกรูป **โค้ดโหลดรูปของ 2 ไฟล์นี้เป็นคนละเส้นทางกันโดยสิ้นเชิง ทดสอบไฟล์หนึ่งผ่านไม่ได้แปลว่าอีกไฟล์ผ่าน**

**ทางแก้ (frontend ล้วน ใน `report.html` ไฟล์เดียว ไม่แตะ backend):** เขียน `attachPhotoLoaders()` ใหม่ — (1) **lazy load** ด้วย `IntersectionObserver` (rootMargin 200px) โหลดเฉพาะรูปที่เลื่อนมาใกล้จอ (2) **จำกัดคิวพร้อมกัน 5 รูป** (`PHOTO_MAX_CONCURRENT`) ผ่าน queue + pump (3) ข้ามรูปที่หลุดจาก DOM แล้ว (`img.isConnected === false`) กรณีกดค้นหาใหม่ทับ (4) fallback สำหรับเบราว์เซอร์เก่าที่ไม่มี IntersectionObserver → โหลดทุกรูปแต่ผ่านคิวจำกัดจำนวนเหมือนกัน

**ผลวัดจริงบน production หลัง deploy (เงื่อนไขเดิมเป๊ะ 6–31 ก.ค. = 303 แถว):**

| | ก่อนแก้ | หลังแก้ |
|---|---|---|
| เวลาแสดงตาราง | 6.7 วิ | **1.8 วิ** |
| คำขอโหลดรูปตอนเปิด | 303 พร้อมกัน | **0** (โหลดเมื่อเลื่อนถึง) |
| เวลาต่อคำขอ | มัธยฐาน 21.5 วิ / สูงสุด 39.7 วิ | **มัธยฐาน 2.3 วิ / สูงสุด 3.3 วิ** |
| คำขอพร้อมกันสูงสุด | 303 | **5 (ตามที่ตั้งไว้)** |
| สภาพหน้าเว็บ | ค้างจนไม่ตอบสนอง | ใช้งานลื่นตลอด |

ยืนยันด้วยภาพหน้าจอจริงว่ารูปย่อยังขึ้นครบตามที่เลื่อนถึง กดดูรูปใหญ่ได้ปกติ — **Cleanup:** รีเซ็ต `pin_hash`/`pin_fail_count`/`pin_locked_until` ของบัญชีทดสอบหัวหน้ากลับเป็น null/0/null เรียบร้อย ไม่มีข้อมูลทดสอบตกค้าง

**บทเรียนสำหรับ agent ตัวถัดไป:** (1) เวลาทดสอบฟีเจอร์ที่มีทั้งฝั่ง auth (`dashboard.html`) และฝั่ง PIN (`report.html`) **ต้องทดสอบทั้ง 2 ไฟล์เสมอ** เพราะบางกลไก (โดยเฉพาะรูปภาพ) ใช้คนละเส้นทางกันสิ้นเชิง — เป็นคู่แฝดคนละสายเลือดกับบทเรียนข้อ 33.3 (2) ฟีเจอร์ที่ "ยิงคำขอต่อ 1 แถวของตาราง" ต้องคิดเรื่องจำนวนแถวสูงสุดที่เป็นไปได้จริงเสมอ (ที่นี่คือค้นทั้งเดือน = 300+ แถว และจะโตขึ้นทุกเดือน) ควรใส่ lazy load + จำกัด concurrency ตั้งแต่แรก

---

## 38. ระบบ "เหลื่อมเวลา" ระดับกลุ่มงาน (migration 32) — ✅ backend + frontend + E2E เสร็จ (20 ส.ค. 2569)

**สถานะ: SQL รันบน production แล้ว + E2E 13 เคสผ่านหมด + แก้ frontend ครบ 3 ไฟล์ + `node --check` ผ่าน — ยังไม่ push (รอเจ้าของ confirm)**

**ที่มา:** เจ้าของหยิบแผนเดิมเรื่องกลุ่มงานครอบครัว/ธุรกิจขึ้นมาทำต่อ โจทย์คือ "เช็กอินได้สองชุด ชุดแรกต้องมาก่อน 08:30 เพื่อพร้อมรับผู้มาติดต่อ ส่วนคนอื่นในกลุ่มเหลื่อมเวลาได้เล็กน้อย" — grill กับเจ้าของ 2 รอบ (8 คำถาม) ก่อนเริ่มเขียนโค้ด

**⚠️ รอบนี้กลับดีไซน์เดิมโดยตั้งใจ** — `CLAUDE_ARCHIVE.md` ข้อ 22 เคยยืนยันว่า "ไม่แตะสีรายคนเลย ใช้ badge ทีมแทน" (ทำไปแล้วเป็น P1a/ข้อ 30) แต่โจทย์รอบนี้ต้องแตะสีรายคน ไม่งั้นคนชุดหลังก็ยังได้ส้ม/แดงเหมือนเดิม = ไม่มีอะไรเปลี่ยน เจ้าของรับทราบและเลือกทางนี้เอง

### 38.1 กติกาที่เจ้าของยืนยัน (อย่าตีความเอง)

1. **เปิด/ปิดรายกลุ่ม** — `work_group.stagger_enabled` หัวหน้าติ๊กเองใน dashboard (deploy ครั้งแรกปิดหมดทั้ง 6 กลุ่ม = ไม่มีอะไรเปลี่ยนทันที)
2. **ไม่ต้องตั้งว่าใครอยู่ชุดไหน** — ระบบดูจากเวลาเช็กอินจริง คนแรกของกลุ่มในวันนั้นคือ "ชุดแรก" โดยอัตโนมัติ กลุ่มสลับเวรกันเองได้ทุกวันโดยไม่ต้องแจ้งใคร
3. **ปลดล็อกเมื่อ "คนอื่น" ในกลุ่มเช็กอินสมบูรณ์ทันก่อน `core_open_before` ครบ `core_min_count` คน** (default 08:30 / 1 คน ปรับต่อกลุ่มได้)
4. **ผ่อนผัน `settings.stagger_minutes`** (default 30) เลื่อนเกณฑ์ทั้ง เขียว/เหลือง/ส้ม
5. **ห้ามเขียนสีทับเงียบๆ** — ติดป้าย `⏱ เหลื่อมเวลา` ทุกที่ + เก็บ `check_in.stagger_applied` ไว้ให้ P2 (ระบบคะแนน) แยกสถิติได้
6. **badge การ์ด "แผนกงานการให้บริการ" (`team_ok_before` 08:20) แยกขาด ไม่แตะ** — เจ้าของเลือกเองว่า badge = รางวัลมาเร็ว ส่วน 08:30 = เส้นตายเปิดบริการ คนละความหมาย
7. **สถิติรายเดือน** นับรวมเป็นสีปกติ + มีบรรทัดย่อย "⏱ ได้ผ่อนผันเหลื่อมเวลา (นับรวมในสีด้านบนแล้ว)" ซ่อนเมื่อเป็น 0

### 38.2 ⚠️ 2 จุดตัดสินใจเชิงตรรกะที่สำคัญ (อย่าแก้กลับโดยไม่อ่าน)

**(ก) ไม่กั้นด้วย `v_local >= core_open_before`** — ตอนออกแบบแรกคิดว่าจะเช็กการผ่อนผันเฉพาะคนที่มาหลังเส้นตาย แต่จะเกิดบั๊ก **"ยิ่งมาสายยิ่งได้สีดีกว่า"**: คนมา 08:25 ได้เหลือง (เกณฑ์ปกติ) แต่คนมา 08:35 ได้เขียว (เกณฑ์ +30) — เลยใช้เงื่อนไขเดียวคือ "มีคนอื่นเช็กอินทันก่อนเส้นตายแล้วกี่คน" ซึ่ง **monotonic** (พอครบแล้วครบตลอดวัน เพราะเวลาเดินหน้าอย่างเดียว) ⇒ ลำดับสีไม่มีทางกลับหัว

**(ข) `stagger_applied = true` เฉพาะเมื่อ "เปลี่ยนสีจริง"** — คำนวณสีฐาน (เกณฑ์ปกติ) กับสีหลังผ่อนผัน แล้วเทียบกัน ถ้าเท่ากันแปลว่าคนนั้นมาเช้าพออยู่แล้ว ไม่ได้ใช้สิทธิ์ ⇒ **ไม่ติดป้าย** กันป้าย `⏱` ขึ้นมั่วกับคนที่มาตรงเวลาปกติ และทำให้ `stagger_count` ในสถิติมีความหมายจริง

**(ค) coverage ไม่นับ `incomplete_checkin = true`** — เช็กอินไกลเกิน 50 ม./ไม่เปิด GPS ไม่ถือว่ามาเปิดเคาน์เตอร์
**(ง) นับเฉพาะ `officer_id <> ตัวเอง`** — คนเปิดเคาน์เตอร์ไม่มีทางผ่อนผันให้ตัวเอง และวันที่กลุ่มมีคนเข้าเวรคนเดียว ตัวนับเป็น 0 เสมอ ⇒ โดนเกณฑ์ปกติโดยอัตโนมัติ **ไม่ต้องเขียนโค้ดพิเศษสำหรับเคสนี้**

### 38.3 ของที่แก้ (backend)

- Schema: 3 คอลัมน์ใน `work_group`, 1 ใน `settings`, 1 ใน `check_in`
- `do_check_in_impl` (signature เดิม), RPC อ่านข้อมูล 4 ตัว + `do_get_my_month_stats_impl` (signature เดิมทุกตัว)
- DROP+recreate 3 ตัวเพราะเปลี่ยน parameter list/return columns: `do_admin_list_work_groups`, `do_admin_update_work_group`, `do_set_settings` — verify `count(*)=1` + `has_function_privilege('authenticated')` ครบทุกตัว
- **ปิดช่องว่างเดิมไปด้วย:** `do_supervisor_get_history_impl` (ประวัติฝั่ง PIN ของ report.html) **ไม่เคยมี `daily_rank`** มาตั้งแต่ P1b ทั้งที่ `do_get_history` ฝั่ง Auth มี — ยืนยันจาก `pg_get_functiondef` จริง แก้ให้แล้วรอบนี้เพราะแก้ไฟล์เดียวกันอยู่แล้ว

### 38.4 ของที่แก้ (frontend — เพิ่มอย่างเดียว ไม่ลบบรรทัดใดที่ใช้งานอยู่)

- `index.html`: กล่อง `⏱ วันนี้คุณอยู่ชุดเหลื่อมเวลา` (2 จุด result/already) + แถวสถิติ `⏱ ได้ผ่อนผันเหลื่อมเวลา`
- `dashboard.html`: ป้าย `⏱` ในตารางวันนี้/ประวัติ + คอลัมน์ "เหลื่อมเวลา" และปุ่ม `⏱ เปิด/แก้เหลื่อมเวลา` ในการ์ดจัดการกลุ่มงาน + ช่อง "เลื่อนเกณฑ์สีให้ (นาที)" ในการ์ดตั้งค่าระบบ + คอลัมน์ใน Export Excel
- `report.html`: ป้าย `⏱` + คอลัมน์ใน Export Excel (อ่านอย่างเดียวตามเดิม ไม่มี UI จัดการ)
- **ตารางวันนี้ของ dashboard query `check_in` ตรง ไม่ผ่าน RPC** จึงต้องเติม `stagger_applied` ใน `.select()` ด้วย (จุดที่พลาดง่าย)

### 38.5 E2E test — 13 เคส ผ่านหมด (ไม่แตะข้อมูลจริงเลย)

**เทคนิคใหม่ที่ใช้ได้ผลดีมาก: ห่อทั้งชุดทดสอบใน `begin; ... rollback;`** สร้างกลุ่ม/เจ้าหน้าที่ทดสอบ เรียก `do_check_in` จริง แล้ว rollback ทิ้ง — production ไม่มีร่องรอยเลย (verify หลังทดสอบ: ไม่มี officer/group ทดสอบตกค้าง, `stagger_minutes` กลับเป็น 30, `check_in` 553 แถวเท่าเดิม) และเพราะ `now()` คงที่ตลอดทรานแซกชัน จึงจำลอง "มาสายกี่โมง" ได้ด้วยการขยับเกณฑ์แทนการรอเวลาจริง

เคสที่ผ่าน: ปิดสวิตช์ = แดงเหมือนเดิม · เปิดแต่ไม่มีคนทัน = แดง · มีคนทัน+90 = เขียว+ติดป้าย · coverage เป็น incomplete = ไม่นับ · coverage มาหลังเส้นตาย = ไม่นับ · ต้องการ 2 คนแต่ทันแค่ 1 = ไม่ปลดล็อก · ไม่มีกลุ่ม = ไม่กระทบ · +30 สีไม่เปลี่ยน = ไม่ติดป้าย · +80 = เหลือง · +70 = ส้ม · `do_get_today_status` คืนค่าถูก · `stagger_count` ถูก · กันเช็กอินซ้ำวันเดียวกันยังทำงาน

### 38.6 ⚠️ เรื่องที่ต้องให้เจ้าของทำเอง ก่อนเปิดใช้จริง

1. **แจ้งเจ้าหน้าที่ทุกคนก่อนเปิดสวิตช์** — ตอนนี้มี 6 กลุ่ม อีก 4 กลุ่มจะเห็นว่าครอบครัว/ธุรกิจได้เกณฑ์ต่างออกไป ต้องมีเหตุผลที่พูดได้ (ประเด็นความเป็นธรรมที่ท้วงไว้ตั้งแต่ archive ข้อ 22)
2. เปิดสวิตช์ให้ **งานครอบครัว + งานธุรกิจ** เองจากหน้า dashboard (ไม่ต้อง deploy ใหม่)
3. ตัดสินใจเรื่อง **4 คนที่ยังไม่มีกลุ่ม** (จอย/เม่/ปาย/แพร) — ไม่มีกลุ่ม = ไม่มีสิทธิ์ผ่อนผันทุกกรณี
